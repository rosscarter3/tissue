...
void InflationDeflationStresses::derivs(Tissue &T, DataMatrix &cellData,
                                        DataMatrix &wallData,
                                        DataMatrix &vertexData,
                                        DataMatrix &cellDerivs,
                                        DataMatrix &wallDerivs,
                                        DataMatrix &vertexDerivs) {
    double s0 = parameter(0);
    double s1 = parameter(1);
    double s2 = parameter(2);
    double sa = parameter(3);
    double t0 = parameter(4);
    double t1 = parameter(5);
    double t2 = parameter(6);
    double t3 = parameter(7);

    size_t bl = variableIndex(0, 0);
    size_t br = variableIndex(0, 1);
    size_t tl = variableIndex(0, 2);
    size_t tr = variableIndex(0, 3);

    double s = 0.0;

    if (_time < t0) {
        s = s0;
    } else if (_time > t0 && _time < t1) {
        s = s0 + (s1 - s0) / (t1 - t0) * (_time - t0);
    } else if (_time > t1 && _time < t2) {
        s = s1;
    } else if (_time > t2 && _time < t3) {
        s = s1 - (s1 - s2) * (_time - t2) / (t3 - t2);
    } else {
        s = s2;
    }
...
void VertexNoUpdateFromPosition::derivs(Tissue &T, DataMatrix &cellData,
                                        DataMatrix &wallData,
                                        DataMatrix &vertexData,
                                        DataMatrix &cellDerivs,
                                        DataMatrix &wallDerivs,
    ...
    for (size_t i = 0; i < numVertices; ++i)
        if ((parameter(1) > 0 && vertexData[i][posIndex] > parameter(0)) ||
            ...
            for (size_t d = 0; d < dimension; ++d) vertexDerivs[i][d] = 0.0;
}
...
void VertexNoUpdateFromIndex::derivs(Tissue &T, DataMatrix &cellData,
                                     DataMatrix &wallData,
                                     DataMatrix &vertexData,
                                     DataMatrix &cellDerivs,
                                     DataMatrix &wallDerivs,
    ...
    for (size_t i = 0; i < numVariableIndex(0); ++i) {
        ...
        for (size_t d = 0; d < dimension; ++d) vertexDerivs[k][d] = 0.0;
    }
}
...
VertexNoUpdateFromIndexHoldX::VertexNoUpdateFromIndexHoldX(
    ...
    //
    if (paraValue.size() != 0) {
        std::cerr << "VertexNoUpdateFromIndexHoldX::"
    ...
    // Set the variable values
    //
    setId("VertexNoUpdateFromIndexHoldX");
    setParameter(paraValue);
    setVariableIndex(indValue);
...
void VertexNoUpdateFromIndexHoldX::derivs(Tissue &T, DataMatrix &cellData,
                                          DataMatrix &wallData,
                                          DataMatrix &vertexData,
                                          DataMatrix &cellDerivs,
                                          DataMatrix &wallDerivs,
...
VertexNoUpdateFromIndexHoldY::VertexNoUpdateFromIndexHoldY(
    ...
    //
    if (paraValue.size() != 0) {
        std::cerr << "VertexNoUpdateFromIndexHoldY::"
    ...
    // Set the variable values
    //
    setId("VertexNoUpdateFromIndexHoldY");
    setParameter(paraValue);
    setVariableIndex(indValue);
...
void VertexNoUpdateFromIndexHoldY::derivs(Tissue &T, DataMatrix &cellData,
                                          DataMatrix &wallData,
                                          DataMatrix &vertexData,
                                          DataMatrix &cellDerivs,
                                          DataMatrix &wallDerivs,
...
void VertexNoUpdateFromList::derivs(Tissue &T, DataMatrix &cellData,
                                    DataMatrix &wallData,
                                    DataMatrix &vertexData,
                                    DataMatrix &cellDerivs,
                                    DataMatrix &wallDerivs,
    ...
    for (size_t i = 0; i < numVertices; ++i)
        if (std::find(updateVertices.begin(), updateVertices.end(), i) ==
            updateVertices.end())
            for (size_t d = 0; d < dimension; ++d) vertexDerivs[i][d] = 0.0;
}

void VertexNoUpdateFromList::update(Tissue &T, DataMatrix &cellData,
                                    DataMatrix &wallData,
...
VertexRandTip::VertexRandTip(std::vector<double> &paraValue,
    ...
    if (paraValue.size() != 1) {
        std::cerr << "VertexRandTip::"
                  << "VertexRandTip() "
                  << "Uses one parameter for max_randomization angle." << std::endl;
        exit(0);
    }
...
void VertexRandTip::derivs(Tissue &T, DataMatrix &cellData,
                           DataMatrix &wallData, DataMatrix &vertexData,
                           DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                           DataMatrix &vertexDerivs) {}

void VertexRandTip::update(Tissue &T, DataMatrix &cellData,
                           DataMatrix &wallData, DataMatrix &vertexData,
                           double h) {
    ...
    for (size_t cellInd = 0; cellInd < numCells; ++cellInd) {
        ...
        if (cellData[cellInd][rotIndex] != 1 && wallsBackground == 3) {
            ...
            cellData[cellInd][mtIndex + 1] =
                rot[1][0] * vtmp[0] + rot[1][1] * vtmp[1];
...
void VertexNoUpdateBoundary::derivs(Tissue &T, DataMatrix &cellData,
                                    DataMatrix &wallData,
                                    DataMatrix &vertexData,
                                    DataMatrix &cellDerivs,
                                    DataMatrix &wallDerivs,
...
VertexNoUpdateBoundaryPtemplate::  // BB
    ...
    if (indValue.size() != 1) {
        std::cerr << "VertexNoUpdateBoundaryPtemplate::"
                  << "VertexNoUpdateBoundaryPtemplate() "
                  << "Start of additional Cell variable indices (center(x,y,z) at "
                     "first level "
                  << std::endl;
        exit(0);
    }
...
void VertexNoUpdateBoundaryPtemplate::derivs(Tissue &T, DataMatrix &cellData,
                                             DataMatrix &wallData,
                                             DataMatrix &vertexData,
                                             DataMatrix &cellDerivs,
                                             DataMatrix &wallDerivs,
    ...
    for (size_t vertexIndex = 0; vertexIndex < numVertices;
         ++vertexIndex)  // check all the vertices
        if (T.vertex(vertexIndex)
                .isBoundary(T.background())) {  // if it is at the boundary
            size_t numVertexWalls =
                T.vertex(vertexIndex)
                    .numWall();  // take the number of walls connected to it
            for (size_t wallIndexVertex = 0; wallIndexVertex < numVertexWalls;
                 wallIndexVertex++) {  // for each of those walls
                size_t wallIndex = T.vertex(vertexIndex)
                                       .wall(wallIndexVertex)
                                       ->index();  // take global index
                if (T.wall(wallIndex).cell1() == T.background() ||
                    T.wall(wallIndex).cell2() ==
                        T.background()) {  // if the wall is boundary
                    size_t cellIndex;

                    if (T.wall(wallIndex).cell1() ==
                        T.background())  // find the cell which is not background
                    ...
                    double COM[3] = {
                        cellData[cellIndex][comIndex], cellData[cellIndex][comIndex + 1],
                        cellData[cellIndex][comIndex + 2]};  // take the position of COM

                    double CV[3] = {
                        vertexData[vertexIndex][0] - COM[0],
                    ...
                    double wallVector[3] = {
                        vertexData[v1][0] - vertexData[v2][0],
                    ...
                    if (temp < 0.00000000001)
                        std::cerr << "VertexNoUpdateBoundaryPtemplate::derivs(), strange "
                                     "wall length at the boundary";
                    else
                        for (size_t i = 0; i < 3; i++)  // normalize wall vector
                            wallVector[i] /= temp;
                    temp = CV[0] * wallVector[0] + CV[1] * wallVector[1] +
                           CV[2] * wallVector[2];
                    double edgeNormal[3] = {
                        CV[0] - temp * wallVector[0], CV[1] - temp * wallVector[1],
                    ...
                    temp =
                        vertexDerivs[vertexIndex][0] *
                            edgeNormal[0] +  // projection of vertex derivs on edge normal
            ...
            //           take the coordinates of the cell-centerCOM which is not
            //           background
            //               calculate the vector Vertex-COM
            //               calculate wallvector
            //               calculate
            //               A=(vertex-COM)-((wallvector).(Vertex-COM))wallvector/norm(wallvector)
...
VertexNoUpdateBoundaryPtemplateStatic::  // BB
    ...
    if (indValue.size() != 1) {
        std::cerr << "VertexNoUpdateBoundaryPtemplateStatic::"
                  << "VertexNoUpdateBoundaryPtemplateStatic() "
                  << "Start of Cell COM indices at first level " << std::endl;
        exit(0);
    }
    // Set the variable values
    //
...
void VertexNoUpdateBoundaryPtemplateStatic::initiate(
    ...
    for (size_t vertexIndex = 0; vertexIndex < numVertices;
        ...
        if (T.vertex(vertexIndex)
            ...
            size_t numVertexWalls =
                T.vertex(vertexIndex)
                    .numWall();  // take the number of walls connected to it
            // std::cerr<<"boundary vertex "<< vertexIndex  <<" has "<< numVertexWalls
            // << " walls"<< std::endl;
            std::vector<double> cellNormal(3);
            boundaryNormal.push_back(
                cellNormal);  // alocate space for the normal vector
            size_t numVertexWallBoundary =
                0;  // counter for number of boundary walls of the vertex
            for (size_t wallIndexVertex = 0; wallIndexVertex < numVertexWalls;
                 wallIndexVertex++) {  // for each of those walls
                size_t wallIndex = T.vertex(vertexIndex)
                                       .wall(wallIndexVertex)
                ...
                if (T.wall(wallIndex).cell1() == T.background() ||
                    ...
                    size_t cellIndex;
                    if (T.wall(wallIndex).cell1() ==
                        T.background())  // find its cell (which is not background)
                        cellIndex = T.wall(wallIndex).cell2()->index();
                    else
                        cellIndex = T.wall(wallIndex).cell1()->index();
                    // std::cerr<<" boundary wall "<< wallIndex  <<" is in the cell "<<
                    // cellIndex
                    //          << std::endl;
                ...
                // std::cerr<<"here.................................."<<numBoundaryVertices<<std::endl;;
            }
...
void VertexNoUpdateBoundaryPtemplateStatic::derivs(
    Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix
        &vertexDerivs) {  // std::cerr<< numBoundaryVertices <<"  "
    ...
    // std:: cerr<<"derives.................................... "
    for (size_t vertex = 0; vertex < numBoundaryVertices;
         vertex++) {  // for the boundary vertices
        ...
        if (norLength > 0.1) {
            ...
            std::cerr << "strange normal length....................." << std::endl;
    }
}
...
void VertexNoUpdateBoundaryPtemplateStatic3D::initiate(
    Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs) {
    size_t numCells = T.numCell();
    ...
    for (size_t cellIndex = 0; cellIndex < numCells;
        ...
        if (cellData[cellIndex][neighborIndex] == -5) {  // if it is at the side
            ...
            for (size_t verInd = 0; verInd < numCellVertices; verInd++) {
                ...
                if (verIndPlus == numCellVertices) verIndPlus = 0;
                size_t verGInd = T.cell(cellIndex).vertex(verInd)->index();
                size_t verGIndPlus = T.cell(cellIndex).vertex(verIndPlus)->index();
                normal[0] += (vertexData[verGInd][1] - com[1]) *
                                 (vertexData[verGIndPlus][2] - com[2]) -
                             (vertexData[verGInd][2] - com[2]) *
                                 (vertexData[verGIndPlus][1] - com[1]);
                normal[1] += (vertexData[verGInd][2] - com[2]) *
                                 (vertexData[verGIndPlus][0] - com[0]) -
                             (vertexData[verGInd][0] - com[0]) *
                                 (vertexData[verGIndPlus][2] - com[2]);
                normal[2] += (vertexData[verGInd][0] - com[0]) *
                                 (vertexData[verGIndPlus][1] - com[1]) -
                             (vertexData[verGInd][1] - com[1]) *
                                 (vertexData[verGIndPlus][0] - com[0]);
            }
            double tmp = std::sqrt(normal[0] * normal[0] + normal[1] * normal[1] +
                                   normal[2] * normal[2]);
...
void VertexNoUpdateBoundaryPtemplateStatic3D::derivs(
    Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs) {
    // size_t dimension = vertexData[0].size();

    for (size_t sideInd = 0; sideInd < numSideCells;
         sideInd++) {  // for the side cells
        ...
        for (size_t verInd = 0; verInd < N; verInd++) {
            ...
            double temp = vertexDerivs[verGInd][0] * sideNormals[sideInd][0] +
    ...
    for (size_t bottomInd = 0; bottomInd < numBottomCells;
         bottomInd++) {  // for the bottom cells
        size_t N = T.cell(bottomNormals[bottomInd][3]).numVertex();
        for (size_t verInd = 0; verInd < N; verInd++) {
            size_t verGInd =
                T.cell(bottomNormals[bottomInd][3]).vertex(verInd)->index();
            double temp = vertexDerivs[verGInd][0] * bottomNormals[bottomInd][0] +
...
void VertexNoUpdateBoundary3D::initiate(Tissue &T, DataMatrix &cellData,
                                        DataMatrix &wallData,
                                        DataMatrix &vertexData,
                                        DataMatrix &cellDerivs,
                                        DataMatrix &wallDerivs,
            ...
            for (size_t j = i + 1; j < N; j++)
                if (tmpsisters[j][2] == 0) {
                    ...
                    for (size_t k = 0; k < M; ++k) {
                        if (tmpsisters[j][0] == sisters[counter - 1][k]) ww += 1;
                        if (tmpsisters[j][1] == sisters[counter - 1][k]) ww += 2;
                    }
                    if (ww == 1) sisters[counter - 1].push_back(tmpsisters[j][1]);
                    if (ww == 2) sisters[counter - 1].push_back(tmpsisters[j][0]);
                    if (ww != 0) tmpsisters[j][2] = counter;
                }
            counter++;
        }
...
void VertexNoUpdateBoundary3D::derivs(Tissue &T, DataMatrix &cellData,
                                      DataMatrix &wallData,
                                      DataMatrix &vertexData,
                                      DataMatrix &cellDerivs,
                                      DataMatrix &wallDerivs,
    ...
    // for (size_t sideInd=0; sideInd<numSideVertices; sideInd++){//for the side
    // cells
    ...
    for (size_t bottomInd = 0; bottomInd < numBottomVertices;
         bottomInd++) {  // for the bottom cells
...
void VertexFromConstStressBoundary::initiate(Tissue &T, DataMatrix &cellData,
                                             DataMatrix &wallData,
                                             DataMatrix &vertexData,
                                             DataMatrix &cellDerivs,
                                             DataMatrix &wallDerivs,
    ...
    for (size_t vIndex = 0; vIndex < numVertices; ++vIndex) {
        if (vertexData[vIndex][0] > rx - epcilon &&
            vertexData[vIndex][0] < rx + epcilon) {
            tmprightVertices[0].push_back(vIndex);
            tmprightVertices[1].push_back(vertexData[vIndex][1]);
        }
        if (vertexData[vIndex][0] > lx - epcilon &&
            vertexData[vIndex][0] < lx + epcilon) {
            tmpleftVertices[0].push_back(vIndex);
            tmpleftVertices[1].push_back(vertexData[vIndex][1]);
        }
        if (vertexData[vIndex][1] > ty - epcilon &&
            vertexData[vIndex][1] < ty + epcilon) {
            tmptopVertices[0].push_back(vIndex);
            tmptopVertices[1].push_back(vertexData[vIndex][0]);
        }
        if (vertexData[vIndex][1] > by - epcilon &&
            vertexData[vIndex][1] < by + epcilon) {
    ...
    for (size_t i = 1; i < tmprightVertices[0].size(); ++i)
        rightVertices[1].push_back(tmprightVertices[1][i - 1] -
                                   tmprightVertices[1][i]);
    for (size_t i = 1; i < tmpleftVertices[0].size(); ++i)
        leftVertices[1].push_back(tmpleftVertices[1][i - 1] -
                                  tmpleftVertices[1][i]);
    ...
    for (size_t i = 1; i < tmpbottomVertices[0].size(); ++i)
        bottomVertices[1].push_back(tmpbottomVertices[1][i - 1] -
                                    tmpbottomVertices[1][i]);

    for (size_t i = 0; i < tmprightVertices[0].size() - 1; ++i)
        rightVertices[2].push_back(tmprightVertices[1][i] -
                                   tmprightVertices[1][i + 1]);
    for (size_t i = 0; i < tmpleftVertices[0].size() - 1; ++i)
        leftVertices[2].push_back(tmpleftVertices[1][i] -
                                  tmpleftVertices[1][i + 1]);
    ...
    for (size_t i = 0; i < tmpbottomVertices[0].size() - 1; ++i)
        bottomVertices[2].push_back(tmpbottomVertices[1][i] -
                                    tmpbottomVertices[1][i + 1]);
...
void VertexFromConstStressBoundary::derivs(Tissue &T, DataMatrix &cellData,
                                           DataMatrix &wallData,
                                           DataMatrix &vertexData,
                                           DataMatrix &cellDerivs,
                                           DataMatrix &wallDerivs,
    ...
    double deltaY =
        vertexData[topVertices[0][0]][1] - vertexData[bottomVertices[0][0]][1];
    double deltaX =
        vertexData[rightVertices[0][0]][0] - vertexData[leftVertices[0][0]][0];
...
void VertexFromConstStressBoundary::update(Tissue &T, DataMatrix &cellData,
                                           DataMatrix &wallData,
                                           DataMatrix &vertexData, double h) {
    size_t numVertices = T.numVertex();
    // size_t dimension = vertexData[0].size();
...
void manipulate::initiate(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
    ...
    for (size_t i = 0; i < numCells; ++i) {
        ...
        if (cellData[i][34] != 0 && cellData[i][34] != 7) {
            ...
            cellData[i][2] = 0;
            double tmp = std::sqrt(cellData[i][0] * cellData[i][0] +
    ...
    // for (size_t i=0; i<numCells; ++i){
    //   double
    //   Radius=std::sqrt((cellData[i][centerIndex]-xx)*(cellData[i][centerIndex]-xx)
...
void manipulate::derivs(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                        DataMatrix &vertexData, DataMatrix &cellDerivs,
                        DataMatrix &wallDerivs, DataMatrix &vertexDerivs)

{}

void manipulate::update(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                        DataMatrix &vertexData, double h) {
    // size_t numVertices = T.numVertex();
    // size_t numCells = T.numCell();
    // size_t dimension = vertexData[0].size();
...
void cellPolarity3D::initiate(Tissue &T, DataMatrix &cellData,
                              DataMatrix &wallData, DataMatrix &vertexData,
                              DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                              DataMatrix &vertexDerivs) {
    ...
    size_t num3dCells = 0;
    for (size_t cellIndex = 0; cellIndex < numCells;
         ++cellIndex)  // find the number of 3d cells
...
void cellPolarity3D::derivs(Tissue &T, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                            DataMatrix &vertexDerivs) {
...
void cellPolarity3D::update(Tissue &T, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            double h) {
    ...
    for (size_t CellInd3d = 0; CellInd3d < num3dCells; ++CellInd3d) {
        totalarea = 0;
        for (size_t i = 0; i < 6; ++i) tmpCentPol[i] = 0;
        ...
        if (cellData[cellFaces[CellInd3d][0]][Lid] == 1) {  // if 3dCell is in L1
            ...
            for (size_t wallIndex = 0; wallIndex < numWalls; ++wallIndex)
                if (cellData[cellFaces[CellInd3d][wallIndex]][wallId] ==
                    0) {  // if the wall is anticlinal

                    area = T.cell(cellFaces[CellInd3d][wallIndex])
                               .calculateVolume(vertexData);
                    totalarea += area;

                    tmpCentPol[0] +=
                        cellData[cellFaces[CellInd3d][wallIndex]][comIndex] * area;
                    tmpCentPol[1] +=
                        cellData[cellFaces[CellInd3d][wallIndex]][comIndex + 1] * area;
                    tmpCentPol[2] +=
                        cellData[cellFaces[CellInd3d][wallIndex]][comIndex + 2] * area;
                    double tmp =
                        cellData[cellFaces[CellInd3d][wallIndex]][polMesInd] * area;

                    tmpCentPol[3] +=
                        tmp * cellData[cellFaces[CellInd3d][wallIndex]][polVecInd];
                    tmpCentPol[4] +=
                        tmp * cellData[cellFaces[CellInd3d][wallIndex]][polVecInd + 1];
                    tmpCentPol[5] +=
                        tmp * cellData[cellFaces[CellInd3d][wallIndex]][polVecInd + 2];
                }
        }

        if (cellData[cellFaces[CellInd3d][0]][Lid] == 2 ||
            cellData[cellFaces[CellInd3d][0]][Lid] ==
                3) {  // if 3dCell is in L2 or L3
            size_t numWalls = cellFaces[CellInd3d].size();
            for (size_t wallIndex = 0; wallIndex < numWalls;
                 ++wallIndex) {  // for all the cell walls
...
//       size_t numWalls=cellFaces[CellInd3d].size();
//       for (size_t wallIndex=0; wallIndex<numWalls; ++wallIndex)
//         if (cellData[cellFaces[CellInd3d][wallIndex]][wallId]==0) { // if the
//         wall is anticlinal
//           numSides++;
...
//     if (cellData[cellFaces[CellInd3d][0]][Lid]==2 ||
//         cellData[cellFaces[CellInd3d][0]][Lid]==3){// if 3dCell is in L2 or
//         L3
//       size_t numWalls=cellFaces[CellInd3d].size();
//       for (size_t wallIndex=0; wallIndex<numWalls; ++wallIndex) { // for all
//       the cell walls
//           numSides++;
//           tmpCentPol[0]+=cellData[cellFaces[CellInd3d][wallIndex]][comIndex
//           ];
...
void cellPolarity3D::printState(Tissue *T, DataMatrix &cellData,
                                DataMatrix &wallData, DataMatrix &vertexData,
                                std::ostream &os) {  // VTK style

    double RR =
        12;  // pol vectors are shown inside this radius(to exclude boundary)
    ...
    myfile << "<DataArray type=\"Int32\" Name=\"connectivity\" format=\"ascii\">"
           << std::endl;
    for (size_t cIndex = 0; cIndex < numPoints; cIndex++) myfile << cIndex << " ";
    ...
    myfile << "<DataArray type=\"Float64\" Name=\"PolarityVector\" "
    ...
    for (size_t vIndex = 0; vIndex < numPoints; vIndex++) {
        ...
        if (radius < RR) {
            ...
            for (size_t d = 0; d < dimension; d++) myfile << 0 << " ";
            myfile << std::endl;
        }
    }
...
diffusion3D::  // BB
    ...
    if (paraValue.size() != 1) {
        std::cerr << "diffusion3D::"
                  << "diffusion3D() "
                  << "Uses one parameter, diffusion constant." << std::endl;
        exit(0);
    }
...
void diffusion3D::initiate(Tissue &T, DataMatrix &cellData,
                           DataMatrix &wallData, DataMatrix &vertexData,
                           DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                           DataMatrix &vertexDerivs) {
    ...
    // for (size_t cellInd=0; cellInd<numCells; ++cellInd)//change cell variable
    // for 3dCell 432
    ...
    for (size_t cellInd = 0; cellInd < numCells;
         ++cellInd) {  // change cell variable for 3dCell 432
    ...
    for (size_t cellInd = 0; cellInd < numCells;
         ++cellInd)  // check all the cells for number of 3d cells
    ...
    for (size_t i = 0; i < num3dCells;
         ++i)  // reserve first component for number of cell walls
        Cells3d[i].push_back(0);

    for (size_t cellInd = 0; cellInd < numCells;
         ++cellInd)  // putting wall(cell) indices into the vector
    ...
    for (size_t i = 0; i < num3dCells; ++i) {
        ...
        for (size_t k = 0; k < tmpVec.size(); ++k) Cells3d[i].push_back(tmpVec[k]);
    }
...
void diffusion3D::derivs(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                         DataMatrix &vertexData, DataMatrix &cellDerivs,
                         DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {
...
void VertexTranslateToMax::initiate(Tissue &T, DataMatrix &cellData,
                                    DataMatrix &wallData,
                                    DataMatrix &vertexData,
                                    DataMatrix &cellDerivs,
                                    DataMatrix &wallDerivs,
    ...
    for (size_t i = 1; i < numVertices; ++i)
        if (vertexData[i][posIndex] > max) max = vertexData[i][posIndex];
    double delta = max - parameter(0);
    for (size_t i = 0; i < numVertices; ++i) vertexData[i][posIndex] -= delta;
}

void VertexTranslateToMax::derivs(Tissue &T, DataMatrix &cellData,
                                  DataMatrix &wallData, DataMatrix &vertexData,
                                  DataMatrix &cellDerivs,
                                  DataMatrix &wallDerivs,
                                  DataMatrix &vertexDerivs) {}

void VertexTranslateToMax::update(Tissue &T, DataMatrix &cellData,
                                  DataMatrix &wallData, DataMatrix &vertexData,
                                  double h) {
    size_t numVertices = T.numVertex();
    size_t posIndex = variableIndex(0, 0);
...
void CenterCOM::initiate(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                         DataMatrix &vertexData, DataMatrix &cellDerivs,
                         DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {
    update(T, cellData, wallData, vertexData, 0.0);
}

void CenterCOM::derivs(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                       DataMatrix &vertexData, DataMatrix &cellDerivs,
                       DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {}

void CenterCOM::derivsWithAbs(Tissue &T, DataMatrix &cellData,
                              DataMatrix &wallData, DataMatrix &vertexData,
                              DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                              DataMatrix &vertexDerivs, DataMatrix &sdydtCell,
                              DataMatrix &sdydtWall, DataMatrix &sdydtVertex) {}

void CenterCOM::update(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                       DataMatrix &vertexData, double h) {
    size_t dimension = vertexData[0].size();
    size_t numVertices = T.numVertex();
...
void CenterCOMcenterTriangulation::initiate(Tissue &T, DataMatrix &cellData,
                                            DataMatrix &wallData,
                                            DataMatrix &vertexData,
                                            DataMatrix &cellDerivs,
                                            DataMatrix &wallDerivs,
...
void CenterCOMcenterTriangulation::derivs(Tissue &T, DataMatrix &cellData,
                                          DataMatrix &wallData,
                                          DataMatrix &vertexData,
                                          DataMatrix &cellDerivs,
                                          DataMatrix &wallDerivs,
...
void CenterCOMcenterTriangulation::update(Tissue &T, DataMatrix &cellData,
                                          DataMatrix &wallData,
                                          DataMatrix &vertexData, double h) {
    size_t dimension = vertexData[0].size();
    size_t numVertices = T.numVertex();
...
CalculatePCAPlane::CalculatePCAPlane(
    ...
    if (paraValue.size() != 1) {
        std::cerr << "CalculatePCAPlane::"
                  << "CalculatePCAPlane() "
                  << "Uses one parameter: onlyInUpdateFlag " << std::endl;
        exit(0);
    }
    if (indValue.size() != 0) {
        std::cerr << "CalculatePCAPlane::"
...
void CalculatePCAPlane::initiate(Tissue &T, DataMatrix &cellData,
                                 DataMatrix &wallData, DataMatrix &vertexData,
                                 DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                 DataMatrix &vertexDerivs) {
    if (parameter(0) == 1.0) {
...
void CalculatePCAPlane::derivs(Tissue &T, DataMatrix &cellData,
                               DataMatrix &wallData, DataMatrix &vertexData,
                               DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                               DataMatrix &vertexDerivs) {
    if (parameter(0) != 1.0) {
...
void CalculatePCAPlane::update(Tissue &T, DataMatrix &cellData,
                               DataMatrix &wallData, DataMatrix &vertexData,
                               double h) {
    if (parameter(0) == 1.0) {
        size_t numCell = T.numCell();
...
void InitiateWallLength::initiate(Tissue &T, DataMatrix &cellData,
                                  DataMatrix &wallData, DataMatrix &vertexData,
                                  DataMatrix &cellDerivs,
                                  DataMatrix &wallDerivs,
                                  DataMatrix &vertexDerivs)

{
    ...
    for (size_t i = 0; i < numWall; ++i) {
        ...
        for (size_t d = 0; d < dimension; ++d)
            distance += (vertexData[v2I][d] - vertexData[v1I][d]) *
                        (vertexData[v2I][d] - vertexData[v1I][d]);
        distance = std::sqrt(distance);
        wallData[i][0] = parameter(0) * distance;
    }
}

void InitiateWallLength::derivs(Tissue &T, DataMatrix &cellData,
                                DataMatrix &wallData, DataMatrix &vertexData,
                                DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                DataMatrix &vertexDerivs) {}
...
void InitiateTargetArea::initiate(Tissue &T, DataMatrix &cellData,
                                  DataMatrix &wallData, DataMatrix &vertexData,
                                  DataMatrix &cellDerivs,
                                  DataMatrix &wallDerivs,
                                  DataMatrix &vertexDerivs)

{
    ...
    for (size_t ii = 0; ii < numCells; ii++) {
        cellData[ii][variableIndex(0, 0)] =
            factor * T.cell(ii).calculateVolume(vertexData);
    }
}

void InitiateTargetArea::derivs(Tissue &T, DataMatrix &cellData,
                                DataMatrix &wallData, DataMatrix &vertexData,
                                DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                DataMatrix &vertexDerivs) {}
...
InitiateWallVariableConstant::InitiateWallVariableConstant(
    ...
    if (indValue.size() != 1 || indValue[0].size() != 1) {
        std::cerr << "InitiateWallVariableConstant::"
                  ...
                  << "One variable index at first level used, the variable to be "
                     "initiated"
                  << std::endl;
        exit(0);
    }
...
void InitiateWallVariableConstant::initiate(Tissue &T, DataMatrix &cellData,
                                            DataMatrix &wallData,
                                            DataMatrix &vertexData,
                                            DataMatrix &cellDerivs,
                                            DataMatrix &wallDerivs,
...
void InitiateWallVariableConstant::derivs(Tissue &T, DataMatrix &cellData,
                                          DataMatrix &wallData,
                                          DataMatrix &vertexData,
                                          DataMatrix &cellDerivs,
                                          DataMatrix &wallDerivs,
...
InitiateWallMesh::InitiateWallMesh(std::vector<double> &paraValue,
    ...
    if (paraValue.size() != 1) {
        std::cerr << "InitiateWallMesh::InitiateWallMesh() "
                  << "Uses one parameter, #of added vertices per wall."
                  << std::endl;
        exit(0);
    }
    if (indValue.size() != 0) {
...
void InitiateWallMesh::initiate(Tissue &T, DataMatrix &cellData,
                                DataMatrix &wallData, DataMatrix &vertexData,
                                DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                DataMatrix &vertexDerivs) {
    size_t dimension = vertexData[0].size();
    ...
    for (size_t i = 0; i < numWallOld; ++i) {
        ...
        T.wall(numWall + numMesh - 1)
            .setVertex1(T.vertexP(numVertex + numMesh - 1));
...
void InitiateWallMesh::derivs(Tissue &T, DataMatrix &cellData,
                              DataMatrix &wallData, DataMatrix &vertexData,
                              DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                              DataMatrix &vertexDerivs) {}
...
StrainTest::StrainTest(std::vector<double> &paraValue,
    ...
    if (paraValue.size() < 2) {
        std::cerr << "StrainTest::StrainTest() "
                  << "Uses at least two parameters, version, angle and optional "
                     "parameters."
                  << std::endl;
        exit(0);
    }
...
void StrainTest::initiate(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                          DataMatrix &vertexData, DataMatrix &cellDerivs,
                          DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {}

void StrainTest::derivs(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                        DataMatrix &vertexData, DataMatrix &cellDerivs,
                        DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {
    size_t dimension = vertexData[0].size();
    ...
    if (parameter(0) == 0.0) {
        if (numParameter() != 2) {
            std::cerr << "StrainTest::derivs() version " << parameter(0)
                      << " requires " << 2 << " parameters." << std::endl;
            exit(-1);
        }
        for (size_t d = 0; d < dimension; ++d) {
...
void CalculateVertexStressDirection::initiate(Tissue &T, DataMatrix &cellData,
                                              DataMatrix &wallData,
                                              DataMatrix &vertexData,
                                              DataMatrix &cellDerivs,
                                              DataMatrix &wallDerivs,
    ...
    if (parameter(0) == 1.0) {
        ...
        for (size_t i = 0; i < numVertex; ++i) {
            T.vertex(i).calculateStressDirection(vertexData, wallData,
                                                 wallForceIndexes_);
        }
    }
}

void CalculateVertexStressDirection::derivs(Tissue &T, DataMatrix &cellData,
                                            DataMatrix &wallData,
                                            DataMatrix &vertexData,
                                            DataMatrix &cellDerivs,
                                            DataMatrix &wallDerivs,
...
void CalculateVertexStressDirection::update(Tissue &T, DataMatrix &cellData,
                                            DataMatrix &wallData,
                                            DataMatrix &vertexData, double h) {
    if (parameter(0) == 1.0) {
        size_t numVertex = T.numVertex();
...
void MoveVerticesRandomlyCapCylinder::initiate(Tissue &T, DataMatrix &cellData,
                                               DataMatrix &wallData,
                                               DataMatrix &vertexData,
                                               DataMatrix &cellDerivs,
                                               DataMatrix &wallDerivs,
...
void MoveVerticesRandomlyCapCylinder::derivs(Tissue &T, DataMatrix &cellData,
                                             DataMatrix &wallData,
                                             DataMatrix &vertexData,
                                             DataMatrix &cellDerivs,
                                             DataMatrix &wallDerivs,
...
void MoveVerticesRandomlyCapCylinder::update(Tissue &T, DataMatrix &cellData,
                                             DataMatrix &wallData,
                                             DataMatrix &vertexData, double h) {
}
...
void scaleTemplate::initiate(Tissue &T, DataMatrix &cellData,
                             DataMatrix &wallData, DataMatrix &vertexData,
                             DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                             DataMatrix &vertexDerivs) {
    size_t numVertices = T.numVertex();
    ...
    for (size_t VertexIndex = 0; VertexIndex < numVertices; ++VertexIndex) {
        ...
        // if(vertexData[ VertexIndex][1]<0)
        //   vertexData[ VertexIndex][1]=-std::pow(-vertexData[
        //   VertexIndex][1],0.75);
    ...
    //   for (size_t d=0; d<dimension; ++d )
    //     distance +=
    //     (vertexData[v2I][d]-vertexData[v1I][d])*(vertexData[v2I][d]-vertexData[v1I][d]);
    //   distance = std::sqrt(distance);
    //   wallData[i][0] = parameter(0)*distance;
    // }
}

void scaleTemplate::derivs(Tissue &T, DataMatrix &cellData,
                           DataMatrix &wallData, DataMatrix &vertexData,
                           DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                           DataMatrix &vertexDerivs) {}

void scaleTemplate::update(Tissue &T, DataMatrix &cellData,
                           DataMatrix &wallData, DataMatrix &vertexData,
                           double h) {}
...
void copyCellVector::initiate(Tissue &T, DataMatrix &cellData,
                              DataMatrix &wallData, DataMatrix &vertexData,
                              DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                              DataMatrix &vertexDerivs) {
    size_t numCells = T.numCell();
    // copy vectors
    for (size_t cellInd = 0; cellInd < numCells; ++cellInd) {
        cellData[cellInd][variableIndex(0, 1)] =
            cellData[cellInd][variableIndex(0, 0)];
        cellData[cellInd][variableIndex(0, 1) + 1] =
            cellData[cellInd][variableIndex(0, 0) + 1];
        cellData[cellInd][variableIndex(0, 1) + 2] =
            cellData[cellInd][variableIndex(0, 0) + 2];
        cellData[cellInd][variableIndex(0, 1) + 3] =
            cellData[cellInd][variableIndex(0, 0) + 3];
    }
}

void copyCellVector::derivs(Tissue &T, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                            DataMatrix &vertexDerivs) {}

void copyCellVector::update(Tissue &T, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            double h) {}
...
randomizeMT::randomizeMT(std::vector<double> &paraValue,
    ...
    if (paraValue.size() != 4) {
        std::cerr
            ...
            << "for concentration(0/1) randomization in the range of "
...
void randomizeMT::initiate(Tissue &T, DataMatrix &cellData,
                           DataMatrix &wallData, DataMatrix &vertexData,
                           DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                           DataMatrix &vertexDerivs) {
    size_t numCells = T.numCell();
    ...
    for (size_t cellInd = 0; cellInd < numCells; ++cellInd) {
        ...
        for (size_t k = 1; k < numVer - 1; ++k) {
            ...
            temp[0] = (vertexData[ind2][1] - vertexData[ind1][1]) *
                          (vertexData[ind3][2] - vertexData[ind1][2]) -
                      (vertexData[ind2][2] - vertexData[ind1][2]) *
                          (vertexData[ind3][1] - vertexData[ind1][1]);
            temp[1] = (vertexData[ind2][2] - vertexData[ind1][2]) *
                          (vertexData[ind3][0] - vertexData[ind1][0]) -
                      (vertexData[ind2][0] - vertexData[ind1][0]) *
                          (vertexData[ind3][2] - vertexData[ind1][2]);
            temp[2] = (vertexData[ind2][0] - vertexData[ind1][0]) *
                          (vertexData[ind3][1] - vertexData[ind1][1]) -
                      (vertexData[ind2][1] - vertexData[ind1][1]) *
                          (vertexData[ind3][0] - vertexData[ind1][0]);
        ...
        // double teta=(parameter(0)*3.1415/180)*(1-2*((double) rand() /
        // (RAND_MAX)));
        ...
        double tmp = std::sqrt(randMT[0] * randMT[0] + randMT[1] * randMT[1] +
                               randMT[2] * randMT[2]);
        randMT[0] /= tmp;
        randMT[1] /= tmp;
        randMT[2] /= tmp;
...
void randomizeMT::derivs(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                         DataMatrix &vertexData, DataMatrix &cellDerivs,
                         DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {}

void randomizeMT::update(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                         DataMatrix &vertexData, double h) {}
...
void restrictVertexRadially::derivs(Tissue &T, DataMatrix &cellData,
                                    DataMatrix &wallData,
                                    DataMatrix &vertexData,
                                    DataMatrix &cellDerivs,
                                    DataMatrix &wallDerivs,
    ...
    for (size_t VertexIndex = 0; VertexIndex < numVertices; ++VertexIndex) {
        ...
        tmp = (vertexDerivs[VertexIndex][0] * tmp1 +
               vertexDerivs[VertexIndex][1] * tmp2);
        vertexDerivs[VertexIndex][0] = tmp * tmp1;
        vertexDerivs[VertexIndex][1] = tmp * tmp2;
    }
}

void restrictVertexRadially::update(Tissue &T, DataMatrix &cellData,
                                    DataMatrix &wallData,
                                    DataMatrix &vertexData, double h) {}
...
CreationPrimordiaTime::CreationPrimordiaTime(
    ...
    if (indValue.size() != 1 || indValue[0].size() != 2) {
        std::cerr
            ...
            << "as first indexand COM index in the second level" << std::endl;
        exit(0);
    }
...
void CreationPrimordiaTime::derivs(Tissue &T, DataMatrix &cellData,
                                   DataMatrix &wallData, DataMatrix &vertexData,
                                   DataMatrix &cellDerivs,
                                   DataMatrix &wallDerivs,
                                   DataMatrix &vertexDerivs) {
    ...
    if (parameter(4) == 1)
        ...
        for (size_t cellI = 0; cellI < proCells.size(); ++cellI)
            cellDerivs[proCells[cellI]][cIndex] +=
                parameter(0) / T.cell(cellI).calculateVolume(vertexData);
...
void CreationPrimordiaTime::update(Tissue &T, DataMatrix &cellData,
                                   DataMatrix &wallData, DataMatrix &vertexData,
                                   double h) {
    ...
    if (timeT > n * parameter(1)) {
        ...
        for (size_t i = 1; i < nCells; ++i)
            if (cellData[i][comInd + 2] > maxZ) maxZ = cellData[i][comInd + 2];

        size_t pInd = 0;
        double distance = 100000;

        for (size_t i = 1; i < nCells; ++i)
            if (cellData[i][comInd + 2] < maxZ - parameter(3) &&
                cellData[i][comInd + 2] > maxZ - parameter(3) - deltaz) {
                tmp = 3.1415 * fmod(n * parameter(2), 360) / 180;
                double tmpDistance =
                    (cellData[i][comInd + 2] - maxZ + parameter(3)) *
                        (cellData[i][comInd + 2] - maxZ + parameter(3)) +
                    (cellData[i][comInd + 1] - R * std::sin(tmp)) *
                        (cellData[i][comInd + 1] - R * std::sin(tmp)) +
                    (cellData[i][comInd] - R * std::cos(tmp)) *
                        (cellData[i][comInd] - R * std::cos(tmp));
        ...
        if (pInd != 0) proCells.push_back(pInd);

        n++;
        // std::cerr<<tmp*180/3.1415<<" "<<std::cos(tmp)<<"
        // "<<std::sin(tmp)<<std::endl; std::cerr<<"end primordium "; for(size_t
        // i=0; i< proCells.size(); ++i)
        //   std::cerr<<proCells[i]<<"  ";
        // std::cerr<<std::endl;
    }
}
...
void VertexFromRotationalForceLinear::initiate(Tissue &T, DataMatrix &cellData,
                                               DataMatrix &wallData,
                                               DataMatrix &vertexData,
                                               DataMatrix &cellDerivs,
                                               DataMatrix &wallDerivs,
...
void VertexFromRotationalForceLinear::derivs(Tissue &T, DataMatrix &cellData,
                                             DataMatrix &wallData,
                                             DataMatrix &vertexData,
                                             DataMatrix &cellDerivs,
                                             DataMatrix &wallDerivs,
    ...
    for (size_t v = 0; v < UpN; ++v)
        for (size_t d = 0; d < 3; ++d) vertexDerivs[boundVerticesUp[v][3]][d] = 0;
    for (size_t v = 0; v < DnN; ++v)
        for (size_t d = 0; d < 3; ++d) vertexDerivs[boundVerticesDn[v][3]][d] = 0;
}

void VertexFromRotationalForceLinear::update(Tissue &T, DataMatrix &cellData,
                                             DataMatrix &wallData,
                                             DataMatrix &vertexData, double h) {
    ...
    double rot[3][3] = {{0, 0, 0}, {0, 1, 0}, {0, 0, 0}};
    ...
    for (size_t v = 0; v < UpN; ++v)
        for (size_t d = 0; d < 3; ++d)
            vertexData[boundVerticesUp[v][3]][d] =
                ...
                rot[d][2] * (boundVerticesUp[v][2] - centerUp[2]) + +centerUp[d];
    ...
    for (size_t v = 0; v < DnN; ++v)
        for (size_t d = 0; d < 3; ++d)
            vertexData[boundVerticesDn[v][3]][d] =
                ...
                rot[d][2] * (boundVerticesDn[v][2] - centerDn[2]) + +centerDn[d];
...
ThresholdSwitch::ThresholdSwitch(std::vector<double> &paraValue,
    ...
    if (indValue.size() != 2 || indValue[0].size() != 1 ||
        ...
        std::cerr << "ThresholdSwitch::ThresholdSwitch() "
                  ...
                  << ", and one for a list of variables to change" << std::endl;
        exit(0);
    }
    //
    // Set the variable values
...
void ThresholdSwitch::derivs(Tissue &T, DataMatrix &cellData,
                             DataMatrix &wallData, DataMatrix &vertexData,
                             DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                             DataMatrix &vertexDerivs) {
...
void ThresholdSwitch::derivsWithAbs(
    Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
...
void ThresholdSwitch::update(Tissue &T, DataMatrix &cellData,
                             DataMatrix &wallData, DataMatrix &vertexData,
                             double h) {
    size_t numCells = T.numCell();
...
void ThresholdReset::derivs(Tissue &T, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                            DataMatrix &vertexDerivs) {
    // Nothing to be done for the derivative function.
...
void ThresholdReset::derivsWithAbs(Tissue &T, DataMatrix &cellData,
                                   DataMatrix &wallData, DataMatrix &vertexData,
                                   DataMatrix &cellDerivs,
                                   DataMatrix &wallDerivs,
                                   DataMatrix &vertexDerivs,
...
void ThresholdReset::update(Tissue &T, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            double h) {
    size_t numCells = T.numCell();
...
void ThresholdNoisyReset::derivs(Tissue &T, DataMatrix &cellData,
                                 DataMatrix &wallData, DataMatrix &vertexData,
                                 DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                 DataMatrix &vertexDerivs) {
    // Nothing to be done for the derivative function.
...
void ThresholdNoisyReset::derivsWithAbs(
    Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs, DataMatrix &sdydtCell, DataMatrix &sdydtWall,
    DataMatrix &sdydtVertex) {
...
void ThresholdNoisyReset::update(Tissue &T, DataMatrix &cellData,
                                 DataMatrix &wallData, DataMatrix &vertexData,
                                 double h) {
    size_t numCells = T.numCell();
...
ThresholdResetAndCount::ThresholdResetAndCount(
    ...
    if (paraValue.size() != 2) {
        std::cerr
            << "ThresholdResetAndCount::ThresholdResetAndCount()  parameter used "
            << "Threshold" << std::endl;
        exit(0);
    }
    if (indValue.size() != 2 || indValue[0].size() != 1 ||
...
void ThresholdResetAndCount::derivs(Tissue &T, DataMatrix &cellData,
                                    DataMatrix &wallData,
                                    DataMatrix &vertexData,
                                    DataMatrix &cellDerivs,
                                    DataMatrix &wallDerivs,
...
void ThresholdResetAndCount::derivsWithAbs(
    Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs, DataMatrix &sdydtCell, DataMatrix &sdydtWall,
    DataMatrix &sdydtVertex) {
...
void ThresholdResetAndCount::update(Tissue &T, DataMatrix &cellData,
                                    DataMatrix &wallData,
                                    DataMatrix &vertexData, double h) {
    size_t numCells = T.numCell();
    ...
    for (size_t cellI = 0; cellI < numCells; ++cellI) {
        if (cellData[cellI][cIndex] >= parameter(0)) {
            ...
            rrr = (myRandom::Rnd() - 0.5);
            cellData[cellI][c2Index] = rrr;
            cellData[cellI][13] = cellData[cellI][13] + 1;
        }
    }
}
...
void FlagNoisyReset::derivs(Tissue &T, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                            DataMatrix &vertexDerivs) {
    // Nothing to be done for the derivative function.
...
void FlagNoisyReset::derivsWithAbs(Tissue &T, DataMatrix &cellData,
                                   DataMatrix &wallData, DataMatrix &vertexData,
                                   DataMatrix &cellDerivs,
                                   DataMatrix &wallDerivs,
                                   DataMatrix &vertexDerivs,
...
void FlagNoisyReset::update(Tissue &T, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            double h) {
    size_t numCells = T.numCell();
...
ThresholdAndFlagNoisyReset::ThresholdAndFlagNoisyReset(
    ...
    if (paraValue.size() != 3) {
        std::cerr << "ThresholdAndFlagNoisyReset::ThresholdAndFlagNoisyReset()  "
                  ...
                  << "Threshold, flag value, and noise amplitude for resetting"
                  << std::endl;
        exit(0);
    }
...
void ThresholdAndFlagNoisyReset::derivs(Tissue &T, DataMatrix &cellData,
                                        DataMatrix &wallData,
                                        DataMatrix &vertexData,
                                        DataMatrix &cellDerivs,
                                        DataMatrix &wallDerivs,
...
void ThresholdAndFlagNoisyReset::derivsWithAbs(
    Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs, DataMatrix &sdydtCell, DataMatrix &sdydtWall,
    DataMatrix &sdydtVertex) {
...
void ThresholdAndFlagNoisyReset::update(Tissue &T, DataMatrix &cellData,
                                        DataMatrix &wallData,
                                        DataMatrix &vertexData, double h) {
    size_t numCells = T.numCell();
...
void FlagAddValue::derivs(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                          DataMatrix &vertexData, DataMatrix &cellDerivs,
                          DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {
    // Nothing to be done for the derivative function.
}

void FlagAddValue::derivsWithAbs(Tissue &T, DataMatrix &cellData,
                                 DataMatrix &wallData, DataMatrix &vertexData,
                                 DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                 DataMatrix &vertexDerivs,
                                 DataMatrix &sdydtCell, DataMatrix &sdydtWall,
...
void FlagAddValue::update(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                          DataMatrix &vertexData, double h) {
    size_t numCells = T.numCell();
...
void CopyVariable::derivs(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                          DataMatrix &vertexData, DataMatrix &cellDerivs,
                          DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {
    // Nothing to be done for the derivative function.
}

void CopyVariable::derivsWithAbs(Tissue &T, DataMatrix &cellData,
                                 DataMatrix &wallData, DataMatrix &vertexData,
                                 DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                 DataMatrix &vertexDerivs,
                                 DataMatrix &sdydtCell, DataMatrix &sdydtWall,
...
void CopyVariable::update(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                          DataMatrix &vertexData, double h) {
    size_t numCells = T.numCell();
