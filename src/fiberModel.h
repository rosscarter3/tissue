...
/// The original version including all options is named General, and simplified implementations for Linear 
/// and Hill functions for the feedback is also provided. The Keywork Equilibrium in the reaction names indicate
/// a continuous update towards a target (equilibrium) value.
///
...
namespace FiberModel {
  ...
  /// https://gitlab.com/slcu/teamhj/publications
  ...
  ///
  /// @brief Updates Young's modulii of cells given an anisotropy input using a linear function. 
  /// Update is within update function and updates the mechanics in a single step.
  ///
  /// @details This reaction updates the mechanical properties (Young's modulii) in response to an anisotropy
  /// (or other) input. It uses (reads) an anisotropy value, @f$a@f$, as input (e.g. stress or
  /// strain anisotropy) that should be calculated elsewhere e.g. saved from Mechanical::TRBS::* functions.
  /// This is used to calculate an mechanical (Young's modulus) 'target' anisotropy value using a linear
  /// function
  /// @f[ Y_L = Y_M + 0.5 (1.0+a) Y_F @f]
  /// where @f$Y_L@f$ is the Longitudinal (maximal) Young's modulus, which is updated here, @f$Y_M=p_0,Y_F=p_1@f$ 
  /// are the Matrix (isotropic) and Fiber (anisotropic if @f$a@f$ not equal to 0) material parameters. 
  /// These should be the same as for the mechanical model used. 
  ...
  /// or if initiation of and possible saving of the transverse (minimal) Young's modulus is used 
  /// @verbatim
  /// FiberModel::Linear 2/3 2 1 1[/2]
  /// Y_matrix    # isotropic contribution to material
  /// Y_fiber     # anisotropic contribution to materia
  /// [init_flag] # (0=no initiation, 1=initiate with isotropic, 2=initiate with anisotropy from aniso_index)
  ///
  /// anisotropy_index              # cell anisotropy variable input 
  /// Young_L_index [Young_T_index] # cell variable for saving Y_L [Y_T] @endverbatim
  /// where a second index can be used to store/update also the transverse value of the material given by
  /// @f[ Y_T = Y_M + 0.5 (1.0-a) Y_F @f]
  /// Before the simulation starts, the anisotropy and @f$Y_L@f$ can be initiated: if @f$p_2=0@f$ or not given -
  /// no initiation; @f$p_2=1@f$ - anisotropy is set to zero in all cells and @f$Y_L=Y_M+0.5Y_F@f$; 
  /// @f$p_2=2@f$ -  @f$Y_L@f$ is set to its target value given the anisotropy value in the cell.
  ///
  ...
  /// This can e.g. be used to only update close to mechanical equilibrium by having a threshold on a vertex
  /// velocity calculated and stored by the Calculate::VertexVelocity function. 
  /// In a model file the reaction is defined as
  /// @verbatim
  ...
  /// Y_fiber     # anisotropic contribution to materia
  /// [init_flag] # (0=no initiation, 1=initiate with isotropic, 2=initiate with anisotropy from aniso_index)
  /// [Threshold] # Threshold in read velocity for when updates not done
  ///
  ///  anisotropy_index              # cell anisotropy variable input 
  ///  Young_L_index [Young_T_index] # cell variable for saving Y_L [Y_T] 
  ///  [velocity_index]              # e.g. from "Calculate::VertexVelocity" @endverbatim
  /// @see FiberModel::LinearEquilibrium for same function but with dynamic update towards target value. 
  /// @see VertexFromTRBScenterTriangulationMT and similar functions for calculating strain/stress anisotropy.
  /// @see Calculate::VertexVelocity (for example) to provide vertex velocity values that can block update.
  ///
  ...
  ///
  /// @brief Updates Young's modulii of cells given an anisotropy input using a linear function. Update 
  /// is within update function and the material parameters are updated continuosly towards target value.
  ///
  /// @details This reaction updates the mechanical properties (Young's modulii) in response to an anisotropy
  /// (or other) input. It uses (reads) an anisotropy value, @f$a@f$, as input (e.g. stress or
  /// strain anisotropy) that should be calculated elsewhere e.g. saved from Mechanical::TRBS::* functions.
  ...
  /// function
  /// @f[ Y_L^{target} = Y_M + 0.5 (1.0+a) Y_F @f]
  /// where @f$Y_L@f$ is the Longitudinal (maximal) Young's modulus, which is updated here, @f$Y_M=p_1,Y_F=p_2@f$ 
  /// are the Matrix (isotropic) and Fiber (anisotropic if @f$a@f$ not equal to 0) material parameters. 
  /// These should be the same as for the mechanical model used. A smooth change of the Young' modulus is 
  /// implemented via an Euler step
  /// @f[ Y_L^{new} = Y_L + k_{rate} (Y_L^{target}-Y_L)\Delta t@f]
  /// where @f$k_{rate}=p_0@f$ is the update rate, and @f$Y_L@f$ is read and stored in the given cell index.
  /// In a model file, the most basic form of the reaction is defined as
  /// @verbatim
  ...
  /// Before the simulation starts, the anisotropy and @f$Y_L@f$ can be initiated: if @f$p_3=0@f$ or not given -
  /// no initiation; @f$p_3=1@f$ - anisotropy is set to zero in all cells and @f$Y_L=Y_M+0.5Y_F@f$; 
  /// @f$p_3=2@f$ -  @f$Y_L@f$ is set to its target value given the anisotropy value in the cell.
  ///
  /// In addition, the update will not happen if a cell variable (velocity_index)
  ...
  ///
  /// @brief Updates Young's modulii of cells given an anisotropy input using a Hill function. 
  /// Update is within update function and updates the mechanics in a single step.
  ///
  /// @details This reaction updates the mechanical properties (Young's modulii) in response to an anisotropy
  /// (or other) input. It uses (reads) an anisotropy value, @f$a@f$, as input (e.g. stress or
  /// strain anisotropy) that should be calculated elsewhere e.g. saved from Mechanical::TRBS::* functions.
  /// This is used to calculate an mechanical material (Young's modulus) 'target' anisotropy value using a 
  /// Hill function
  /// @f[ Y_L = Y_M + 0.5(1.0 + \frac{a^n}{(1-a)^{n}K^{n} + a^{n}})Y_F}@f]
  /// where @f$Y_L@f$ is the Longitudinal (maximal) Young's modulus, which is updated here, @f$Y_M=p_0,Y_F=p_1@f$ 
  /// are the Matrix (isotropic) and Fiber (anisotropic if @f$a@f$ not equal to 0) material parameters. 
  ...
  /// anisotropy_index # cell anisotropy variable input 
  /// Young_L_index    # cell variable for saving Y_L @endverbatim
  /// or with added initiation and potential saving of transverse/minimal Young's modulus
  /// @verbatim
  ...
  /// Before the simulation starts, the anisotropy and @f$Y_L@f$ can be initiated: if @f$p_4=0@f$ or not given -
  /// no initiation; @f$p_4=1@f$ - anisotropy is set to zero in all cells and @f$Y_L=Y_M+0.5Y_F@f$; 
  /// @f$p_4=2@f$ - @f$Y_L@f$ is set to its target value given the anisotropy value in the cell.
  ///
  /// In addition, the update will not happen if a cell variable (velocity_index)
  ...
  ///  Young_L_index [Young_T_index] # cell variable for saving Y_L [Y_T] 
  ///  [velocity_index]              # e.g. from "Calculate::VertexVelocity" @endverbatim
  /// @see FiberModel::HillEquilibrium for same function but with dynamic update towards target value. 
  /// @see VertexFromTRBScenterTriangulationMT and similar functions for calculating strain/stress anisotropy.
  /// @see Calculate::VertexVelocity (for example) to provide vertex velocity values that can block update.
  ///
  ...
  ///
  /// @brief Updates Young's modulii of cells given an anisotropy input using a Hill feedback function. Update 
  /// is within update function and the material parameters are updated continuosly towards a target value.
  ///
  /// @details This reaction updates the mechanical properties (Young's modulii) in response to an anisotropy
  /// (or other) input. It uses (reads) an anisotropy value, @f$a@f$, as input (e.g. stress or
  /// strain anisotropy) that should be calculated elsewhere e.g. saved from Mechanical::TRBS::* functions.
  ...
  /// @f[ Y_L^{new} = Y_L + k_{rate} (Y_L^{target}-Y_L)\Delta t@f]
  /// where @f$k_{rate}=p_0@f$ is the update rate, and @f$Y_L@f$ is read and stored in the given cell index.
  /// In a model file, this basic form of the reaction is defined as
  /// @verbatim
  ...
  /// Young_L_index [Young_T_index] # cell variable for saving Y_L [Y_T] @endverbatim
  /// where a second index can be used to store/update also the transverse value of the material given by
  /// @f[ Y_T^{target} = Y_M + 0.5(1.0 - \frac{a^n}{(1-a)^{n}K^{n} + a^{n}})Y_F}@f]
  /// Before the simulation starts, the anisotropy and @f$Y_L@f$ can be initiated: if @f$p_5=0@f$ or not given -
  /// no initiation; @f$p_5=1@f$ - anisotropy is set to zero in all cells and @f$Y_L=Y_M+0.5Y_F@f$; 
  /// @f$p_5=2@f$ -  @f$Y_L@f$ is set to its target value given the anisotropy value in the cell.
  ///
  /// In addition, the update will not happen if a cell variable (velocity_index)
  ...
  /// n_hill      # Hill coefficient
  /// [init_flag] # (0=no initiation, 1=initiate with isotropic, 2=initiate with anisotropy from aniso_index)
  /// [Threshold] # Threshold in read velocity variable for when updates not done
  ///
  /// anisotropy_index              # cell anisotropy variable input 
  /// Young_L_index [Young_T_index] # cell variable for saving Y_L [Y_T] 
  /// [velocity_index]              # e.g. from "Calculate::VertexVelocity" @endverbatim
  /// @see FiberModel::Hill for same function but with direct update to target value.
  /// @see VertexFromTRBScenterTriangulationMT and similar functions for calculating strain/stress anisotropy.
  /// @see Calculate::VertexVelocity (for example) to provide vertex velocity values that can block update.
  ///
