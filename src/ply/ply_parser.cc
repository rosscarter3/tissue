...
bool ply::ply_parser::parse ( std::istream& istream )
{
    ...
    format_type format;
    std::vector< C11NSPACE::shared_ptr<element> > elements;
		...
		if ( !stringstream || !stringstream.eof() ||
		     ...
		     !std::isspace ( space_name_count ) )
		  {
			...
			std::cerr << "s_e_n: " << space_element_name << " name: " << name << " s_n_c: "
                ...
                C11NSPACE::shared_ptr<element> element_ptr ( new element ( name, count, C11NSPACE::get<0> ( element_callbacks ), C11NSPACE::get<1> ( element_callbacks ) ) );
                elements.push_back ( C11NSPACE::shared_ptr<element> ( element_ptr ) );
		    ...
		    // stringstream >> space_type_name >> std::ws >> name >> std::ws;
                    ...
                    std::vector< C11NSPACE::shared_ptr<property> >::const_iterator iterator;
