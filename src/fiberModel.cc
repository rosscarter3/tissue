...
namespace FiberModel {
		   ...
		   std::vector< std::vector<size_t> > &indValue)
  {
    if (paraValue.size() < 2 || paraValue.size() > 4 ) {
      std::cerr << "FiberModel::Linear::Linear() Uses at least 2 and up to 4 parameters:" << std::endl
		<< "young_matrix, young_fiber, (material limits, should be same as for the mechanical "
		<< "(TRBS) model used)" << std::endl
		<< "[initialization_flag] (0 - no initiation; 1 - isotropic; 2 - anisotropic" << std::endl
		<< "[equilibrium_threshold] (only update if vertex moving slower), " << std::endl;
      exit(EXIT_FAILURE);
    }
    ...
    if ( (indValue.size() != 2 && indValue.size() != 3 ) ||
	 ...
	 (indValue.size() == 3 && indValue[2].size() != 1) ) {
      std::cerr << "FiberModel::Linear::Linear() Error in indices given:" << std::endl
		<< "First level gives stress/strain anisotropy index to read." << std::endl
		<< "Second level gives Young_Longitudinal index to update, or Y_L and Y_T indices." << std::endl
		<< "Third optional level gives index for velocity to be compared with p_3." << std::endl;
      exit(EXIT_FAILURE);
    }

    if ( (indValue.size() == 3 && paraValue.size() < 4 ) ||
	 (indValue.size() < 3 && paraValue.size() == 4 ) ) {
      std::cerr << "FiberModel::Linear::Linear() 4th optional [eq_threshold] parameter must be comparable "
		<< "with variable index variable (optional) given at level 3." << std::endl;
      exit(EXIT_FAILURE);
    }
		      ...
		      double h) 
  {
	...
	double youngL = FiberModel::linearFunction(anisotropy,youngMatrix,youngFiber);
	
	if ( youngL>youngMatrix+youngFiber ) // not to exceed maximum when using absolute stress anisotropy 
  ...
  LinearEquilibrium::LinearEquilibrium(std::vector<double> &paraValue,
				       std::vector< std::vector<size_t> > &indValue)
  {
    if (paraValue.size() < 3 || paraValue.size() > 5 ) {
      std::cerr << "FiberModel::LinearEquilibrium::LinearEquilibrium() Uses at least 3 and up to 5 parameters:"
		<< std::endl << "k_rate, (rate of material update (towards target)" << std::endl
		<< "young_matrix, young_fiber, (material limits, should be same as for the mechanical "
		<< "(TRBS) model used)" << std::endl
		<< "[initialization_flag] (0 - no initiation; 1 - isotropic; 2 - anisotropic" << std::endl
		<< "[equilibrium_threshold] (only update if vertex moving slower), " << std::endl;
    ...
    if ( (indValue.size() != 2 && indValue.size() != 3 ) ||
	 ...
	 (indValue[1].size() != 1 && indValue[1].size() != 2) ||
	 (indValue.size() == 3 && indValue[2].size() != 1) ) {
      std::cerr << "FiberModel::LinearEquilibrium::LinearEquilibrium() Error in indices given:" << std::endl
		<< "First level gives stress/strain anisotropy index to read." << std::endl
		<< "Second level gives Young_Longitudinal index to update, or Y_L and Y_T indices." << std::endl
		<< "Third optional level gives index for velocity to be compared with p_4." << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  void LinearEquilibrium::initiate(Tissue &T,
			DataMatrix &cellData,
			DataMatrix &wallData,
			DataMatrix &vertexData,
			DataMatrix &cellDerivs,
  ...
  void LinearEquilibrium::derivs(Tissue &T,
				 DataMatrix &cellData,
				 DataMatrix &wallData,
				 DataMatrix &vertexData,
				 DataMatrix &cellDerivs,
  ...
  void LinearEquilibrium::update(Tissue &T,
				 DataMatrix &cellData,
				 DataMatrix &wallData,
				 DataMatrix &vertexData, 
				 double h) 
  {
	...
	double youngLTarget = FiberModel::linearFunction(anisotropy,youngMatrix,youngFiber);
	
	if ( youngLTarget>youngMatrix+youngFiber ) // not to exceed maximum when using absolute stress anisotropy 
	  youngLTarget = youngMatrix + youngFiber;
	
	double update = kRate*h*(youngLTarget-cellData[cellIndex][YoungLIndex]);
	...
	if (cellData[cellIndex][YoungLIndex]>youngMatrix+youngFiber ) { 
	  std::cerr << "FiberModel::LinearEquilibrium::update ERROR: "
		    << "Update rate too large - Youngs modulus exceded maximal value in cell "
	     ...
	     std::vector< std::vector<size_t> > &indValue)
  {
    if (paraValue.size() < 4 || paraValue.size() > 6 ) {
      std::cerr << "FiberModel::Hill::Hill() Uses at least 4 and up to 6 parameters:" << std::endl
		<< "young_matrix, young_fiber, (material limits, should be same as for the mechanical "
		<< "(TRBS) model used." << std::endl
		<< "K_hill, n_hill, (parameters for feedback function, should be same as for the mechanical "
		<< "(TRBS) model used." << std::endl
		<< "[initialization_flag] (0 - no initiation; 1 - isotropic; 2 - anisotropic" << std::endl
		<< "[equilibrium_threshold] (only update if vertex moving slower), " << std::endl;
      exit(EXIT_FAILURE);
    ...
    if ( (indValue.size() != 2 && indValue.size() != 3 ) ||
	 ...
	 (indValue.size() == 3 && indValue[2].size() != 1) ) {
      std::cerr << "FiberModel::Hill::Hill() Error in indices given:" << std::endl
		<< "First level gives stress/strain anisotropy index to read." << std::endl
		<< "Second level gives Young_Longitudinal index to update, or Y_L and Y_T indices." << std::endl
		<< "Third optional level gives index for velocity to be compared with p_5." << std::endl;
      exit(EXIT_FAILURE);
    }

    if ( (indValue.size() == 3 && paraValue.size() < 6 ) ||
	 (indValue.size() < 3 && paraValue.size() == 6 ) ) {
      std::cerr << "FiberModel::Hill::Hill() 6th optional [eq_threshold] parameter must be comparable "
		<< "with variable index variable (optional) given at level 3." << std::endl;
      exit(EXIT_FAILURE);
    }
		    ...
		    double h) 
  {
	...
	double youngL = FiberModel::hillFunction(anisotropy,youngMatrix,youngFiber,KHill,nHill);
				   ...
				   std::vector< std::vector<size_t> > &indValue)
  {
    if (paraValue.size() < 5 || paraValue.size() > 7 ) {
      std::cerr << "FiberModel::HillEquilibrium::HillEquilibrium() Uses at least 5 and up to 7 parameters:"
		<< std::endl << "k_rate, (rate of update towards target" << std::endl
		<< "young_matrix, young_fiber, (material limits, should be same as for the mechanical "
		<< "(TRBS) model used." << std::endl
		<< "K_hill, n_hill, (parameters for feedback function, should be same as for the mechanical "
		<< "(TRBS) model used." << std::endl
    ...
    if ( (indValue.size() != 2 && indValue.size() != 3 ) ||
	 ...
	 (indValue[1].size() != 1 && indValue[1].size() != 2) ||
	 (indValue.size() == 3 && indValue[2].size() != 1) ) {
      std::cerr << "FiberModel::HillEquilibrium::HillEquilibrium() Error in indices given:" << std::endl
		<< "First level gives stress/strain anisotropy index to read." << std::endl
		<< "Second level gives Young_Longitudinal index to update, or Y_L and Y_T indices." << std::endl
		<< "Third optional level gives index for velocity to be compared with p_6." << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  void HillEquilibrium::initiate(Tissue &T,
			DataMatrix &cellData,
			DataMatrix &wallData,
			DataMatrix &vertexData,
			DataMatrix &cellDerivs,
  ...
  void HillEquilibrium::derivs(Tissue &T,
			       DataMatrix &cellData,
			       DataMatrix &wallData,
			       DataMatrix &vertexData,
			       DataMatrix &cellDerivs,
  ...
  void HillEquilibrium::update(Tissue &T,
			       DataMatrix &cellData,
			       DataMatrix &wallData,
			       DataMatrix &vertexData, 
			       double h) 
  {
	...
	double anisotropy=cellData[cellIndex][AnisoIndex];
	double youngLTarget = FiberModel::hillFunction(anisotropy,youngMatrix,youngFiber,KHill,nHill);
