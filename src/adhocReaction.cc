...
void cellPolarity3D::printState(Tissue *T, DataMatrix &cellData,
  ...
  myfile << "</DataArray>" << std::endl << "</Points>" << std::endl;
...
void limitZdis::initiate(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                         DataMatrix &vertexData, DataMatrix &cellDerivs,
                         DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {
  size_t numCells = T.numCell();
  for (size_t cellIndex = 0; cellIndex < numCells; ++cellIndex)
    if (cellData[cellIndex][38] == -1 &&
        (cellData[cellIndex][37] == -4 ||
         cellData[cellIndex][37] == 0))  // for hypocotyl
    {
      size_t numWalls = T.cell(cellIndex).numWall();
      for (size_t wallindex = 0; wallindex < numWalls; ++wallindex) {
        size_t vInd = T.cell(cellIndex).vertex(wallindex)->index();
        if (vertexData[vInd][2] > -50) topVertices.push_back(vInd);
        if (vertexData[vInd][2] < -50) bottomVertices.push_back(vInd);
      }
    }
}

void limitZdis::derivs(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                       DataMatrix &vertexData, DataMatrix &cellDerivs,
                       DataMatrix &wallDerivs, DataMatrix &vertexDerivs) {
...
void limitZdis::update(Tissue &T, DataMatrix &cellData, DataMatrix &wallData,
                       DataMatrix &vertexData, double h) {}
