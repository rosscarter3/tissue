...
size_t BaseSolver::debugCount() const {
  ...
  if (debugFlag()) return count++ % cellDataCopy_.size();
...
void BaseSolver::setTissueVariables(size_t numCellVariable) {
  ...
  //
  if (cellData_.size() != T_->numCell() || wallData_.size() != T_->numWall() ||
      vertexData_.size() != T_->numVertex()) {
    std::cerr << "BaseSolver::setTissueVariables wrong size of data:"
              << std::endl
              << "Data\tTissue\tInternal" << std::endl
              << "Cell\t" << T_->numCell() << "\t" << cellData_.size()
              << std::endl
              << "Wall\t" << T_->numWall() << "\t" << wallData_.size()
              << std::endl
              << "Vertex\t" << T_->numVertex() << "\t" << vertexData_.size()
              << std::endl;
    exit(-1);
  }
  //
  ...
  } else {  // value given by user, only these cell variables copied (to keep
            // matrix form of cell variables)
  ...
  for (size_t i = 0; i < T_->numWall(); ++i) {
    ...
    assert(T_->wall(i).numVariable() ==
           wallData_[i].size() - 1);  // wallData also stores length
...
void BaseSolver::simulate(size_t verbose) {
  if (verbose) std::cerr << "BaseSolver::simulate(void)\n";
  exit(-1);
}
...
void BaseSolver::print(std::ostream &os) {
  ...
  std::cerr << tCount << " " << t_ << " " << cellData_.size() << " "
            << wallData_.size() << " " << vertexData_.size() << " " << numOk_
            << " " << numBad_ << "  " << t_ - tOld << " "
            << static_cast<int>(cellData_.size()) - static_cast<int>(NOld)
            << " " << numOk_ - okOld << " " << numBad_ - badOld << " " << time
            << std::endl;
  tOld = t_;
  NOld = cellData_.size();
  okOld = numOk_;
  ...
  if (printFlag_ == 0) {
    ...
    size_t dimension =
        T_->vertex(0).numPosition();  // was vertexData_[0].size();
    os << Nv << " " << dimension << std::endl;
    for (size_t i = 0; i < Nv; ++i) {
      for (size_t d = 0; d < dimension; ++d) os << vertexData_[i][d] << " ";
      os << std::endl;
    }
    // os << std::endl;
    ...
    for (size_t i = 0; i < Nc; ++i) {
      ...
      for (size_t k = 0; k < numPrintCellVar;
           ++k) {  // was for( size_t k=0 ; k<cellData_[i].size() ; ++k )
        os << cellData_[i][k] << " ";
      }
    ...
    // Print wall variables, first the two connected vertices and then the
    // variables
  ...
  //
  // Print in vtu format assuming two wall components for wall variables (except
  // for length)
  //
  ...
  //
    ...
    size_t Nc = cellData_.size();
    int numPrintVar =
        T_->cell(0).numVariable() + 3;  // was cellData_[0].size()+3;
    os << Nc << " " << numPrintVar << std::endl;
    for (size_t i = 0; i < Nc; ++i) {
      size_t Ncv = T_->cell(i).numVertex();
      os << Ncv << " ";
  ...
  //
    ...
    size_t Nw = wallData_.size();
    int numPrintVar =
        T_->wall(0).numVariable() + 5;  // was wallData_[0].size()+4;
    os << Nw << " " << numPrintVar << std::endl;
    for (size_t i = 0; i < Nw; ++i) {
      os << "2 ";
      os << T_->wall(i).vertex1()->index() << " "
  ...
  ///
    ...
    for (size_t i = 0; i < Nc; ++i) {
      ...
      for (size_t n = 0; n < numWalls; ++n) {
        double pol = 0.0;
        if (sum != 0.0) pol = cellData_[i][pinI] * Pij[n] / sum;
        os << pol << " ";
      }
      os << std::endl;
    }
  ...
  //
  ...
  else if (printFlag_ == 25) {  // This flag is printing the vtk paired wall and
                                // gnuplot outputs (corresponding to flags 2 and
                                // 5, respectively). The gnuplot output is
                                // written in a file called tissue.gdata.
  ...
  else if (printFlag_ == 27) {  //  This flag is printing displaytissue and
                                //  gnuplot outputs (corresponding to flags 0
                                //  and 5, respectively). The gnuplot output
                                //  does not print the wall variables, and is
                                //  written in a file called tissue.gdata.
  ...
  else if (printFlag_ == 28) {  // Same as flag 27, but generating an extra file
                                // in a displaytissue format for the last time
                                // point.
    // Generating the output in gnuplot format.
    ...
    if (tCount == numPrint_) {
      ...
      for (size_t i = 0; i < Nv; ++i) {
        for (size_t d = 0; d < dimension; ++d) of1 << vertexData_[i][d] << " ";
        of1 << std::endl;
      }
      // os << std::endl;
      // Print the cells, first connected vertecis and then variables
      ...
      for (size_t i = 0; i < Nw; ++i) {
        ...
        for (size_t k = 0; k < wallData_[i].size(); ++k)
          of1 << wallData_[i][k] << " ";
        double distance = T_->wall(i).lengthFromVertexPosition(vertexData_);
        of1 << i << " " << distance << " " << distance - wallData_[i][0] << " "
            << (distance - wallData_[i][0]) / wallData_[i][0] << std::endl;
      }
      of1 << std::endl;
    }
    ...
    os << std::endl;
           ...
           29) {  // Same as flag 26, but printing the vtk format at the last
                  // time point.  but without printing the wall variables in the
                  // output file with gnuplot format. This flag is printing the
                  // vtk paired wall and gnuplot outputs (corresponding to flags
                  // 2 and 5, respectively). The gnuplot output is written in a
                  // file called tissue.gdata.
  ...
  else if (printFlag_ == 30) {  // Same as flag 28, but generating an extra file
                                // in a gnuplot format for the last time point.
    // Generating the output in gnuplot format.
  ...
  else if (printFlag_ == 31) {  // Same as flag 28, but generating a files in
                                // gnuplot and vtk formats for the last time
                                // point.
    // Generating the output in gnuplot format.
  ...
  //
    ...
    size_t NcM = {Nc - 1};
    // os << Nc << " " << numPrintVar << std::endl;
    for (size_t i = 1; i < NcM; ++i) os << cellData_[i][4] << " ";
    for (size_t i = Nc - 1; i < Nc; ++i) os << cellData_[i][4] << std::endl;
  }
  //
  //
  ...
  //
    ...
    os << double(Nc-initialCellNum) / double(initialCellNum) << " " << T_->reaction(0)->parameter(0) << " " << double(numCellInCol)/double(Nc)
       << " " << colonies.size();
  ...
  //
    ...
    os << T_->reaction(0)->parameter(2) << " "  //  poisson(1)
       ...
       << 400 / (vertexData_[2][1] - vertexData_[1][1])
       << " "  //  true stress1 (12)
       << 100 / (vertexData_[1][0] - vertexData_[0][0])
       << std::endl;  //  true stress2 (13)
  }

  else if (printFlag_ == 58) {  // isotropy impact on reducing the stiffness

    os << 1 - (T_->reaction(0)->parameter(1))
       << " "                             //    youngF(as anisotropy messure
    ...
    PVD_file::write(*T_, cellFile, wallFile, tCount);
    os << cellData_[0][25] << " " << cellData_[1][25] << " " << cellData_[2][25]
       << std::endl;
  ...
  else if (printFlag_ == 61) {  // Print in vtu format and angle distribution at
                                // the end PLoS/fig3
    std::string pvdFile = "vtk/tissue.pvd";
    std::string cellFile = "vtk/VTK_cells.vtu";
    std::string wallFile = "vtk/VTK_walls.vtu";
    ...
    if (tCount == numPrint_ - 1) {
      for (size_t cellind = 0; cellind < cellData_.size(); cellind++) {
        if (cellData_[cellind][15] < 75 &&
            cellData_[cellind][15] > -75)  // indices relate to the  model files
          os << cellData_[cellind][12] << " "  //   teta MT
             << cellData_[cellind][13] << " "  //   teta strain
             << cellData_[cellind][14] << " "  //   teta stress
             << cellData_[cellind][15]
             << " "  //   z coordinate ( see reaction
                     //   VertexFromTRBSCenterTri..MT L3910 )
    ...
    os << T_->reaction(0)->parameter(8) << " " << T_->reaction(0)->parameter(5)
       << " " << cellData_[0][19] << " " << cellData_[0][20] << " "
       << cellData_[0][17] + cellData_[1][17] + cellData_[2][17] +
              cellData_[3][17]
       << std::endl;
    //    Teta1     Teta2     isoenergy     anisoenergy    total strain
    //    anisotropy
  }

  else if (printFlag_ == 64) {  // energy landscape for quad for stress feedback
                                // centertriangulated

    os << T_->reaction(0)->parameter(8) << " " << T_->reaction(0)->parameter(10)
       << " " << cellData_[0][19] << " " << cellData_[0][20] << " "
       << cellData_[0][17] + cellData_[1][17] + cellData_[2][17] +
              cellData_[3][17]
       << std::endl;
  ...
  else if (printFlag_ == 65) {  // Print in vtu format and angle distribution at
                                // the end other data for Dorota data
    std::string pvdFile = "vtk/tissue.pvd";
    std::string cellFile = "vtk/VTK_cells.vtu";
    std::string wallFile = "vtk/VTK_walls.vtu";
    static size_t numCellVar = T_->cell(0).numVariable();
      ...
      for (size_t cellind = 0; cellind < cellData_.size(); cellind++) {
        if (cellData_[cellind][54] == 1)
          os << cellData_[cellind][40] << " "  // 1 celldata index
             ...
             << cellData_[cellind][12]
             << " "  // 6 angle between max_stress and MT
             << cellData_[cellind][13]
             << " "  // 7 angle between max_growth and max_strain
             << cellData_[cellind][49] << " "  // 8 max_growth_rate(data)
             << cellData_[cellind][11] << " "  // 9 max_strain
             << cellData_[cellind][54]
             << " "  // 10 info boundary and empty cells
    ...
    os << wallData_[0][0] + wallData_[0][1] + wallData_[0][2] << " "
       ...
       << cellData_[0][42] << " " << cellData_[0][43] << " " << cellData_[0][44]
       << " " << cellData_[0][45] << " " << cellData_[0][46] << " "
       << cellData_[0][47] << " " << cellData_[0][48] << " " << cellData_[0][49]
       << "       " << vertexDerivs_[0][1] << " " << std::endl;

  }

  else if (printFlag_ == 68) {  // Print in vtu format and angle distribution at
                                // the end PLoS/fig3
    std::string pvdFile = "tmp/tissue.pvd";
    ...
    for (size_t wallind = 0; wallind < wallData_.size(); wallind++)
      varWallStress += (avWallStress - wallData_[wallind][2]) *
                       (avWallStress - wallData_[wallind][2]);
    ...
    for (size_t cellind = 0; cellind < cellData_.size(); cellind++)
      varCellStress += (avCellStress - cellData_[cellind][27]) *
                       (avCellStress - cellData_[cellind][27]);
    ...
    //        <<
    //        (wallData_[wallind][1]-wallData_[wallind][0])/wallData_[wallind][0]
    //        <<std::endl; //   growth
  ...
  // updated as well
    ...
    std::vector<int> histogram(
        *(std::max_element(neighbors.begin(), neighbors.end())) + 1, 0);

    for (std::list<int>::const_iterator i = neighbors.begin(),
                                        e = neighbors.end();
         i != e; ++i) {
      ++histogram[*i];
    }
    ...
    for (std::vector<int>::const_iterator i = histogram.begin(),
                                          e = histogram.end();
         i != e; ++i) {
      sum += *i;
    }
  ...
  // For printing internal edges in reaction
  // VertexFromExternalSpringFromPerpVertex Very ad hoc assuming reaction having
  // index 3 in model file (fourth reaction)
  //
  else if (printFlag_ == 101) {
    size_t reactionIndex = 0;
    for (size_t rind = 0; rind < 5; rind++)
      if (T_->reaction(rind)->id() ==
              "VertexFromExternalSpringFromPerpVertex" ||
          T_->reaction(rind)->id() ==
              "VertexFromExternalSpringFromPerpVertexDynamic") {
        reactionIndex = rind;
      }
    if (T_->reaction(reactionIndex)->id() !=
            "VertexFromExternalSpringFromPerpVertex" &&
        T_->reaction(reactionIndex)->id() !=
            "VertexFromExternalSpringFromPerpVertexDynamic") {
      std::cerr
          << "BaseSolver::print() with printFlag 101. Wrong name of reaction "
          << reactionIndex << " in reaction list, no printing." << std::endl;
    } else
      T_->reaction(reactionIndex)
          ->printState(T_, cellData_, wallData_, vertexData_);
    std::string pvdFile = "vtk/tissue.pvd";
    std::string cellFile = "vtk/VTK_cells.vtu";
    std::string wallFile = "vtk/VTK_walls.vtu";
    static size_t numCellVar = T_->cell(0).numVariable();
  ...
  else if (printFlag_ == 102) {  //(adhoc) Print in vtu format and edges to
                                 //study strain and growth
    std::string pvdFile = "vtk/tissue.pvd";
    std::string cellFile = "vtk/VTK_cells.vtu";
    std::string wallFile = "vtk/VTK_walls.vtu";
    static size_t numCellVar = T_->cell(0).numVariable();
    ...
    //   os <<  (Length0-restLength0)/restLength0 <<" " // edge0 strain
    //   (l-l0)/l0
    //      <<  restLength0                     <<" "//  edge0-principalStrain
    //      angle
    //      <<  (Length1-restLength1)/restLength1 <<" "//  edge1 strain
    //      <<  restLength1                     <<" "//  edge1-principalStrain
    //      angle
    ...
    // }
           ...
           103) {  // Print cell and membrane values (e.g. for PIN) at same row
    ...
    for (size_t cellI = 0; cellI < cellData_.size(); ++cellI) {
      ...
      if (T_->cell(cellI).numWall() != 4) {
        std::cerr << "BaseSolver::print(), printFlag=103: Assuming FOUR "
                     "neighbors per cell!"
                  << std::endl;
        exit(EXIT_FAILURE);
      }
      ...
      if (cellI < cellData_.size() - 1) {
        for (size_t wallK = 0; wallK < T_->cell(cellI).numWall(); ++wallK) {
          ...
          if (T_->wall(wallI).cell1()->index() == cellI &&
              T_->wall(wallI).cell2() != T_->background()) {
        ...
        for (size_t wallK = T_->cell(cellI).numWall() - 1;
             wallK < T_->cell(cellI).numWall(); --wallK) {
          size_t wallI = T_->cell(cellI).wall(wallK)->index();
          if (T_->wall(wallI).cell1()->index() == cellI &&
              T_->wall(wallI).cell2() != T_->background()) {
            os << wallData_[wallI][pinIndexWall] << " ";
      ...
      os << std::endl;
      if (neighCount != 2) {
        std::cerr << "BaseSolver::print(), printFlag=103: Did not find two "
                     "neighbors for cell "
                  << cellI << "!" << std::endl;
        exit(EXIT_FAILURE);
      }
    ...
    printInitTri(os);
           ...
           105) {  // Print cell and membrane values (e.g. for PIN) at same row
    size_t auxinIndexCell = 4;
    size_t pinIndexCell = 5;
    size_t pinIndexWall = 3;
    for (size_t cellI = 0; cellI < cellData_.size(); ++cellI) {
  ...
  else if (printFlag_ == 106) {  // for meristem 3D vtu printing seperate
                                 // top,side,bottom; L1,L2,L3
    size_t reactionIndex = 0;
    for (size_t rind = 0; rind < 5; rind++)
    ...
    if (T_->reaction(reactionIndex)->id() !=
      ...
      std::cerr
          << "BaseSolver::print() with printFlag 106. Wrong name of reaction "
          << reactionIndex << " in reaction list, no printing." << std::endl;
    } else
      T_->reaction(reactionIndex)
          ->printState(T_, cellData_, wallData_, vertexData_);
...
void BaseSolver::printInit(std::ostream &os) const {
  ...
  os << T_->numCell() << " " << T_->numWall() << " " << T_->numVertex()
     << std::endl;
  ...
  // Print cell data (only up to number of variables stored in cells in tissue,
  // i.e. not center triangulated ones)
  size_t numCellVar = T_->cell(0).numVariable();
  os << T_->numCell() << " " << numCellVar << std::endl;
  if (T_->cell(0).numVariable()) {
    for (size_t i = 0; i < T_->numCell(); ++i) {
      assert(cellData_[i].size());
      for (size_t j = 0; j < numCellVar; ++j) os << cellData_[i][j] << " ";
      os << std::endl;
    }
    os << std::endl;
  }
  ...
  if (T_->cell(0).numVariable()) {
    for (size_t i = 0; i < T_->numCell(); ++i) {
      ...
      if (cellData_[i].size() == T_->cell(i).numVariable()) {
        ...
        // Print com
        for (size_t d = 0; d < com.size(); ++d) os << com[d] << " ";
        ...
        for (size_t k = 0; k < numInternalWall; ++k) {
          ...
          double distance =
              std::sqrt((com[0] - vertexData_[vertexIndex][0]) *
...
void BaseSolver::printInitFem(std::ostream &os) const {
  ...
  unsigned int oldPrecision = os.precision();
  os.precision(20);
  // std::cerr << "Tissue::printInit(): old precision: " << oldPrecision << "
  // new "
  //				<< os.precision() << std::endl;
...
void BaseSolver::printInitTri(std::ostream &os) const {
  ...
  unsigned int oldPrecision = os.precision();
  os.precision(20);
  // std::cerr << "Tissue::printInitTri(): old precision: " << oldPrecision << "
  // new "
  //	    << os.precision() << std::endl;
  if (T_->cell(0).numVariable() == cellData_[0].size()) {
    std::cerr
        << "BaseSolver::printInitTri() Only works for tissue with central mesh"
  ...
  std::vector<std::pair<size_t, size_t>> cellNeigh(
      numW);  // Wall connections to cells
  std::vector<std::pair<size_t, size_t>> vertexNeigh(
      numW);  // Wall connections to vertices
  ...
  for (size_t i = 0; i < T_->numCell(); ++i) {
    ...
    for (size_t d = 0; d < v[vI].size(); ++d) {
      v[vI][d] = cellData_[i][numCellVar +
                              d];  // Assuming central vertex stored at end
    }
    for (size_t k = 0; k < T_->cell(i).numWall(); ++k) {
      ...
      wallTmpData[0] = cellData_[i][T_->cell(i).numVariable() + D +
                                    k];  // set current length
      w[wI] = wallTmpData;
      vertexNeigh[wI].first = T_->cell(i).vertex(k)->index();
      vertexNeigh[wI].second = vI;
      if (k == 0) {
        cellNeigh[wI].first = cellIndexStart[i] + T_->cell(i).numWall() -
                              2;  // last from added list of new cells
        cellNeigh[wI].second = T_->cell(i).index();  // i
      } else {
        cellNeigh[wI].first =
            cellNeigh[wI - 1].second;  // second from prev wall (going around)
        cellNeigh[wI].second =
            cellIndexStart[i] + k - 1;  // from added list of new cells
      }
    }
  }
  ...
  for (size_t i = 0; i < numW; ++i) {
    ...
    os << vertexNeigh[i].first << " " << vertexNeigh[i].second
       << std::endl;  // vertex neighbors
  }
  os << std::endl;
  ...
  for (size_t i = 0; i < numV; ++i) {
    for (size_t j = 0; j < v[i].size(); ++j) os << v[i][j] << " ";
    os << std::endl;
  }
  os << std::endl;
  ...
  for (size_t i = 0; i < numW; ++i) {
    //     assert( wa[i].size() );
    for (size_t j = 0; j < w[i].size(); ++j) os << w[i][j] << " ";
    os << std::endl;
  }
  os << std::endl;
  ...
  if (c[0].size()) {
    for (size_t i = 0; i < numC; ++i) {
      assert(c[i].size());
      for (size_t j = 0; j < c[i].size(); ++j) os << c[i][j] << " ";
      os << std::endl;
    }
    os << std::endl;
  }
