...
namespace Division {
  ...
  VolumeViaLongestWall::VolumeViaLongestWall(
    ...
    //
    if (paraValue.size() != 3) {
      std::cerr << "Division::VolumeViaLongestWall::"
  ...
  int VolumeViaLongestWall::flag(Tissue *T, size_t i, DataMatrix &cellData,
  ...
  void VolumeViaLongestWall::update(Tissue *T, size_t i, DataMatrix &cellData,
				    DataMatrix &wallData, DataMatrix &vertexData,
    ...
    for (size_t k = 1; k < divCell->numWall(); ++k) {
      double tmpLength =
        divCell->wall(k)->setLengthFromVertexPosition(vertexData);
  ...
  VolumeViaLongestWallCenterTriangulation(
    ...
    if (paraValue.size() != 3 && paraValue.size() != 4) {
      std::cerr << "DivisionVolumeViaLongestWallCenterTriangulation::"
		...
		<< "if double resting length option used" << std::endl;
      exit(EXIT_FAILURE);
    }
    if (indValue.size() != 2 || indValue[1].size() != 2) {
      std::cerr << "DivisionVolumeViaLongestWallCenterTriangulation::"
		<< "DivisionVolumeViaLongestWallCenterTriangulation() "
		<< "Variable indices for volume dependent cell "
		<< "at first level, Start of additional Cell variable indices "
		<< "(center(x,y,z) "
		<< "and Wall length index and at second level" << std::endl;
      exit(EXIT_FAILURE);
    }
    //
    // Set the variable values
    ...
    tmp[1] = "LWall_frac";
    tmp[2] = "LWall_threshold";
    if (numParameter() == 4) tmp[3] = "doubleFlag";
    setParameterId(tmp);
  }
  ...
  update(
	 Tissue *T, size_t i, DataMatrix &cellData, DataMatrix &wallData,
	 DataMatrix &vertexData, DataMatrix &cellDeriv, DataMatrix &wallDeriv,
	 DataMatrix &vertexDeriv) {
    Cell *divCell = &(T->cell(i));
    size_t dimension = vertexData[0].size();
    assert(divCell->numWall() > 1);
    ...
    // Divide
    if (numParameter() == 3)
      T->divideCellCenterTriangulation(
				       divCell, wI, w3I, variableIndex(1, 0), variableIndex(1, 1), v1Pos,
				       v2Pos, cellData, wallData, vertexData, cellDeriv, wallDeriv,
				       vertexDeriv, variableIndex(0), parameter(2), 0);
    if (numParameter() == 4 && parameter(3) == 1)
      T->divideCellCenterTriangulation(
				       divCell, wI, w3I, variableIndex(1, 0), variableIndex(1, 1), v1Pos,
				       v2Pos, cellData, wallData, vertexData, cellDeriv, wallDeriv,
				       vertexDeriv, variableIndex(0), parameter(2), 1);
    assert(numWallTmp + 3 == T->numWall());
  ...
  update(
	...
	if (fac2 != 0.0) {  // else parallell and not applicable
	  ...
	  if (t >= 0.0 && t < 1.0) {  // within wall
	    std::cerr << "wall " << k << " (t=" << t << " " << fac1 << "/" << fac2
		      << ") chosen as second wall" << std::endl;
	    flag++;
	    w3I = k;
	    w3Tmp.push_back(k);
	    w3tTmp.push_back(t);
    ...
    if (flag != 1) {
      std::cerr
        << "Divide::VolumeViaLongestWall3DCenterTriangulation::update() Warning"
        << " more than one wall possible as connection "
        << "for cell " << i << std::endl;
      for (size_t k = 0; k < divCell->numWall(); ++k) {
	std::cerr << "0 " << vertexData[divCell->wall(k)->vertex1()->index()][0]
		  << " " << vertexData[divCell->wall(k)->vertex1()->index()][1]
		  << " " << vertexData[divCell->wall(k)->vertex1()->index()][2]
		  << "\n0 " << vertexData[divCell->wall(k)->vertex2()->index()][0]
	...
	size_t k = w3Tmp[kk];
	std::cerr << "1 " << vertexData[divCell->wall(k)->vertex1()->index()][0]
		  << " " << vertexData[divCell->wall(k)->vertex1()->index()][1]
		  << " " << vertexData[divCell->wall(k)->vertex1()->index()][2]
		  << "\n1 " << vertexData[divCell->wall(k)->vertex2()->index()][0]
		  << " " << vertexData[divCell->wall(k)->vertex2()->index()][1]
		  << " " << vertexData[divCell->wall(k)->vertex2()->index()][2]
		  << std::endl << std::endl << std::endl;		  
      }
      std::cerr << "2 " << vertexData[divCell->wall(wI)->vertex1()->index()][0]
		<< " " << vertexData[divCell->wall(wI)->vertex1()->index()][1]
		<< " " << vertexData[divCell->wall(wI)->vertex1()->index()][2]
		<< std::endl << "2 " << vertexData[divCell->wall(wI)->vertex2()->index()][0]
		<< " " << vertexData[divCell->wall(wI)->vertex2()->index()][1]
		<< " " << vertexData[divCell->wall(wI)->vertex2()->index()][2]
		<< std::endl << std::endl << std::endl;		
      std::cerr << "3 " << vertexData[divCell->wall(w3I)->vertex1()->index()][0]
		<< " " << vertexData[divCell->wall(w3I)->vertex1()->index()][1]
		<< " " << vertexData[divCell->wall(w3I)->vertex1()->index()][2]
		<< std::endl << "3 " << vertexData[divCell->wall(w3I)->vertex2()->index()][0]
	...
	w3tTmp[w3tTmp.size() - 1] *
	(vertexData[v2w3I][d] - vertexData[v1w3I][d]);
    ...
    // Divide
    T->divideCellCenterTriangulation(
				     divCell, wI, w3I, variableIndex(1, 0), variableIndex(1, 1), v1Pos, v2Pos,
				     cellData, wallData, vertexData, cellDeriv, wallDeriv, vertexDeriv,
				     variableIndex(0), parameter(2), 0);
  ...
  Branching::Branching(std::vector<double> &paraValue,
		       std::vector<std::vector<size_t>> &indValue) {
    //
    // Do some checks on the parameters and variable indeces
    //
    if (paraValue.size() != 4) {
      std::cerr << "Division::Branching::"
		<< "Branching() "
		<< "four parameters used firstConc_threshold, "
		<< "secondConc_threshold, LWall_frac, and "
		<< "Lwall_threshold " << std::endl;
      // if( paraValue.size()!=5 ) {
      //   std::cerr << "Branching::"
      //             << "Branching() "
      //             << "five parameters used firstConc_threshold,
      //             secondConc_threshold, LWall_frac, and "
      //             << "Lwall_threshold and branching_wall_bending_stiffness." <<
      //             std::endl;      
      exit(EXIT_FAILURE);
    }
    if (indValue.size() != 1) {
      std::cerr
        << "Division::Branching::"
        << "Branching() "
        << "five variable indices for cell branching flag and first and second "
	<< "concentrations  "
  ...
  int Branching::flag(Tissue *T, size_t i, DataMatrix &cellData,
		      DataMatrix &wallData, DataMatrix &vertexData,
		      DataMatrix &cellDerivs, DataMatrix &wallDerivs,
		      DataMatrix &vertexDerivs) {
    //  if( T->cell(i).calculateVolume(vertexData) > parameter(0) &&
    //  cellData[i][11]==0 ) {
    if (cellData[i][variableIndex(0, 0)] == 0 &&
	cellData[i][variableIndex(0, 1)] > parameter(0) &&
	cellData[i][variableIndex(0, 2)] < parameter(1)) {
      std::cerr << "Cell " << i << " marked for branching " << T->cell(i).volume()
		<< std::endl;
  ...
  void Branching::update(Tissue *T, size_t i, DataMatrix &cellData,
			 DataMatrix &wallData, DataMatrix &vertexData,
			 DataMatrix &cellDeriv, DataMatrix &wallDeriv,
			 DataMatrix &vertexDeriv) {
    ...
    if (dimension > 2) {
      std::cerr << "Division::Branching::"
		<< "Branching() "
		<< "only for two-dimensional tissue " << std::endl;
      exit(EXIT_FAILURE);
    }
    // assert( divCell->numWall() > 1 );
    // assert( dimension==2 || dimension==3 );
    ...
    for (size_t k = 0; k < brCell->numWall(); ++k)
      if ((brCell->wall(k)->cell1() == T->background()) ||
	  (brCell->wall(k)->cell2() == T->background()))
    ...
    double minLength = brCell->wall((wI + 1) % (brCell->numWall()))
      ->setLengthFromVertexPosition(vertexData);
    ...
    for (size_t d = 0; d < dimension; ++d) {
      ...
      v2Pos[d] = (0.3 + minLength / maxLength) * vertexData[v1wI][d] +
	(0.7 - minLength / maxLength) * vertexData[v2wI][d];
    }
    ...
    temp = std::sqrt(wallvecNorm[0] * wallvecNorm[0] +
		     wallvecNorm[1] * wallvecNorm[1]);
    if (temp != 0) {
      wallvecNorm[0] /= temp;
      wallvecNorm[1] /= temp;
    }
    ...
    // std::cerr<<"wallvec norm   "<<wallvecNorm[0]<<"  " <<wallvecNorm[1]
    // <<std::endl;
    //
    ...
    // branching
    T->branchCell(brCell, wI, v1Pos, v2Pos, v3Pos, v4Pos, cellData, wallData,
		  vertexData, cellDeriv, wallDeriv, vertexDeriv, parameter(2));
    ...
    //  std::cerr<< "cell "<< i+1 << "auxin " << cellData[i+1][5]<<"
    //  "<<wallData[numWallTmp][1]<<std::endl;
    //}
    // Check that the division did not mess up the data structure
    // T->checkConnectivity(1);
  }
  ...
  VolumeViaLongestWallSpatial(
    ...
    if (indValue.size() != 2 || indValue[0].size() != 1) {
      std::cerr << "DivisionVolumeViaLongestWallSpatial::"
		<< "DivisionVolumeViaLongestWallSpatial() "
		<< "Spatial index in first level and variable indices for volume "
		<< "dependent cell "
  ...
  flag(Tissue *T, size_t i, DataMatrix &cellData,
	...
	if (vertexData[i][sI] > sMax_) sMax_ = vertexData[i][sI];
    }
  ...
  VolumeViaLongestWall3D(
    ...
    if (paraValue.size() != 3) {
      std::cerr
        << "Division::VolumeViaLongestWall3D::"
        << "VolumeViaLongestWall3D() "
        << "Two parameters used V_threshold, LWall_frac LWall_threshold\n";
      exit(EXIT_FAILURE);
    }
    if (indValue.size() != 1) {
      std::cerr << "Division::VolumeViaLongestWall3D::"
		<< "VolumeViaLongestWall3D() "
		<< "Variable indices for volume dependent cell "
		<< "variables is used." << std::endl;
      exit(EXIT_FAILURE);
    ...
    // Set the variable values
    //
    setId("Division::VolumeViaLongestWall3D");
    setNumChange(1);
    setParameter(paraValue);
    setVariableIndex(indValue);
  ...
  update(Tissue *T, size_t i, DataMatrix &cellData,
	...
	std::cerr << "0 " << vertexData[divCell->wall(k)->vertex1()->index()][0]
		  << " " << vertexData[divCell->wall(k)->vertex1()->index()][1]
		  << " " << vertexData[divCell->wall(k)->vertex1()->index()][2]
		  << std::endl << "0 " << vertexData[divCell->wall(k)->vertex2()->index()][0]
		  << " " << vertexData[divCell->wall(k)->vertex2()->index()][1]
		  << " " << vertexData[divCell->wall(k)->vertex2()->index()][2]
		  << std::endl << std::endl << std::endl;
      }
	...
	std::cerr << "1 " << vertexData[divCell->wall(k)->vertex1()->index()][0]
		  << " " << vertexData[divCell->wall(k)->vertex1()->index()][1]
		  << " " << vertexData[divCell->wall(k)->vertex1()->index()][2]
		  << std::endl << "1 " << vertexData[divCell->wall(k)->vertex2()->index()][0]
		  << " " << vertexData[divCell->wall(k)->vertex2()->index()][1]
		  << " " << vertexData[divCell->wall(k)->vertex2()->index()][2]
		  << std::endl << std::endl << std::endl;
    }
  ...
  VolumeViaShortestWall3D(
    ...
    if (paraValue.size() != 3) {
      std::cerr
        << "Division::VolumeViaShortestWall3D::"
        << "VolumeViaShortestWall3D() "
        << "Two parameters used V_threshold, LWall_frac LWall_threshold\n";
      exit(EXIT_FAILURE);
    }
    if (indValue.size() != 1) {
      std::cerr << "Division::VolumeViaShortestWall3D::"
    << "VolumeViaShortestWall3D() "
    << "Variable indices for volume dependent cell "
    << "variables is used." << std::endl;
      exit(EXIT_FAILURE);
    ...
    // Set the variable values
    //
    setId("Division::VolumeViaShortestWall3D");
    setNumChange(1);
    setParameter(paraValue);
    setVariableIndex(indValue);
  ...
  update(Tissue *T, size_t i, DataMatrix &cellData,
   ...
   DataMatrix &vertexDeriv) {
    ...
    //
    size_t wI = 0;
    double minLength = divCell->wall(0)->setLengthFromVertexPosition(vertexData);
    for (size_t k = 1; k < divCell->numWall(); ++k) {
      double tmpLength =
        divCell->wall(k)->setLengthFromVertexPosition(vertexData);
  ...
  minLength = tmpLength;
    ...
    for (size_t d = 0; d < dimension; ++d) {
      nW[d] = (vertexData[v1wI][d] - vertexData[v2wI][d]) / minLength;
      v1Pos[d] = 0.5 * (vertexData[v1wI][d] + vertexData[v2wI][d]);
    }
  ...
  if (fac2 != 0.0) {  // else parallell and not applicable
    ...
    assert(w3I != divCell->numWall());
    if (flag != 1) {
      std::cerr << "divideVolumeViaShortestWall3D::update() Warning"
    << " more than one wall possible as connection "
    << "for cell " << i << std::endl;
      for (size_t k = 0; k < divCell->numWall(); ++k) {
  std::cerr << "0 " << vertexData[divCell->wall(k)->vertex1()->index()][0]
...
VolumeViaLongestWall3DSpatial::VolumeViaLongestWall3DSpatial(
    std::vector<double> &paraValue,
    std::vector<std::vector<size_t>> &indValue) {
  // Do some checks on the parameters and variable indeces
  //////////////////////////////////////////////////////////////////////
  ...
  if (indValue.size() != 2 || indValue[0].size() != 1) {
    std::cerr << "DivisionVolumeViaLongestWall3DSpatial::"
              << "DivisionVolumeViaLongestWall3DSpatial() "
              << "Variable indices for spatial direction in first and volume "
                 "dependent cell "
...
int VolumeViaLongestWall3DSpatial::flag(
    Tissue *T, size_t i, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs) {
  size_t sI = variableIndex(0, 0);
...
void VolumeViaLongestWall3DSpatial::update(
    Tissue *T, size_t i, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDeriv, DataMatrix &wallDeriv,
    DataMatrix &vertexDeriv) {
  Cell *divCell = &(T->cell(i));
  ...
  if (flag != 1) {
    ...
    std::cerr << "2 " << vertexData[divCell->wall(wI)->vertex1()->index()][0]
              << " " << vertexData[divCell->wall(wI)->vertex1()->index()][1]
              << " " << vertexData[divCell->wall(wI)->vertex1()->index()][2]
              << "\n2 " << vertexData[divCell->wall(wI)->vertex2()->index()][0]
              << " " << vertexData[divCell->wall(wI)->vertex2()->index()][1]
              << " " << vertexData[divCell->wall(wI)->vertex2()->index()][2]
              << "\n\n\n";
    std::cerr << "3 " << vertexData[divCell->wall(w3I)->vertex1()->index()][0]
              << " " << vertexData[divCell->wall(w3I)->vertex1()->index()][1]
              << " " << vertexData[divCell->wall(w3I)->vertex1()->index()][2]
              << "\n3 " << vertexData[divCell->wall(w3I)->vertex2()->index()][0]
              << " " << vertexData[divCell->wall(w3I)->vertex2()->index()][1]
              << " " << vertexData[divCell->wall(w3I)->vertex2()->index()][2]
              << "\n\n\n";
    std::cerr << "4 "
...
void VolumeViaStrain::update(Tissue *T, size_t cellI, DataMatrix &cellData,
  ...
  DataMatrix x(numV), y(numV), dx(numV), xM(numV), yM(numV), dxM(numV);
  ...
  double v = theta;
  if (parameter(3) != 1.0) v = theta - 0.5 * 3.14159;
  n[0] = std::cos(v);
  n[1] = std::sin(v);
  ...
  if (flag != 2) {
    ...
    std::cerr << "2 " << vertexData[divCell->wall(wI[0])->vertex1()->index()][0]
              ...
              << "\n\n\n";
    std::cerr << "3 " << vertexData[divCell->wall(wI[1])->vertex1()->index()][0]
  ...
  T->divideCell(divCell, wI[0], wI[1], v1Pos, v2Pos, cellData, wallData,
                vertexData, cellDeriv, wallDeriv, vertexDeriv, variableIndex(0),
                parameter(2));
  assert(numWallTmp + 3 == T->numWall());
  ...
  VolumeViaDirection::VolumeViaDirection(
					 std::vector<double> &paraValue,
					 std::vector<std::vector<size_t>> &indValue) {
    // Do some checks on the parameters and variable indeces
    //
    if (paraValue.size() != 5) {
      std::cerr
        << "Division::VolumeViaDirection::"
        << "VolumeViaDirection() "
        << "Four parameters used V_threshold, LWall_frac, "
        << "Lwall_threshold, Parallell_flag, and COM (1 = COM, 0 = Random)."
        << std::endl;
      std::exit(EXIT_FAILURE);
    }
    if (indValue.size() != 2 || indValue[0].size() != 1) {
      std::cerr << "Division::VolumeViaDirection::"
		<< "VolumeViaDirection() "
    ...
    // Set the variable values
    //
    setId("Division::VolumeViaDirection");
    setNumChange(1);
    setParameter(paraValue);
    setVariableIndex(indValue);
  ...
  update(Tissue *T, size_t cellI, DataMatrix &cellData,
	 DataMatrix &wallData, DataMatrix &vertexData,
	 DataMatrix &cellDeriv, DataMatrix &wallDeriv,
	 DataMatrix &vertexDeriv) {
    Cell *divCell = &(T->cell(cellI));
	...
	// dimension=3
	if (parameter(3) !=
	    1.0) {  // Perpendicular to given direction, ie plane vector parallell
	  n[0] = cellData[cellI][variableIndex(0, 0)];
	  n[1] = cellData[cellI][variableIndex(0, 0) + 1];
	  ...
	  std::cerr << "Division::VolumeViaDirection::update "
  ...
  VolumeRandomDirection(
    ...
    if (indValue.size() != 1) {
      std::cerr << "Division::VolumeRandomDirection::"
		...
		<< "variables are at first level." << std::endl;
      exit(EXIT_FAILURE);
    }
    // Set the variable values
    //
    setId("Division::VolumeRandomDirection");
    setNumChange(1);
    setParameter(paraValue);
    setVariableIndex(indValue);
  ...
  std::cerr << "DivisionVolumeRandomDirectionCenterTriangulation:: "
            << " is not working adaptation is needed. " << std::endl;
...
int VolumeRandomDirectionCenterTriangulation::flag(
    Tissue *T, size_t i, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs) {
  if (T->cell(i).calculateVolumeCenterTriangulation(
          vertexData, cellData, variableIndex(0, 0)) > parameter(0)) {
    std::cerr << "Cell " << i << " marked for division with volume "
              << T->cell(i).volume() << std::endl;
    return 1;
  }
...
void VolumeRandomDirectionCenterTriangulation::update(
    Tissue *T, size_t cellI, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDeriv, DataMatrix &wallDeriv,
    DataMatrix &vertexDeriv) {
  Cell *divCell = &(T->cell(cellI));
  ...
  for (size_t i = 1; i < numV; i++) {
    std::vector<double> v1pos =
        vertexData[divCell->vertex((b + i - 1) % numV)->index()];
    std::vector<double> v2pos =
        vertexData[divCell->vertex((b + i) % numV)->index()];
  ...
  // size_t e = (b+best)%(numV); // e is the index of the end vertex of division
  // wall
  ...
  if (paraValue.size() != 4) {
    std::cerr << "DivisionForceDirection::ForceDirection() "
              << "Four parameters are used V_threshold, Lwall_fraction, "
                 "Lwall_threshold and orientation_flag."
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
int ForceDirection::flag(Tissue *T, size_t i, DataMatrix &cellData,
                         DataMatrix &wallData, DataMatrix &vertexData,
                         DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                         DataMatrix &vertexDerivs) {
  if (T->cell(i).calculateVolume(vertexData) > parameter(0)) {
...
void ForceDirection::update(Tissue *T, size_t i, DataMatrix &cellData,
                            DataMatrix &wallData, DataMatrix &vertexData,
                            DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                            DataMatrix &vertexDerivs) {
  Cell cell = T->cell(i);
  ...
  T->divideCell(&cell, candidateWalls[0], candidateWalls[1],
                verticesPosition[0], verticesPosition[1], cellData, wallData,
  ...
  VolumeViaShortestPath::VolumeViaShortestPath(
					       std::vector<double> &paraValue,
					       std::vector<std::vector<size_t>> &indValue) {
    // Do some checks on the parameters and variable indeces
    if (paraValue.size() != 3) {
      std::cerr << "Division::VolumeViaShortestPath::"
    ...
    // Set the variable values
    setId("Division::VolumeViaShortestPath");
    setNumChange(1);
    setParameter(paraValue);
    setVariableIndex(indValue);
  ...
  update(Tissue *T, size_t i, DataMatrix &cellData,
	...
	std::vector<double> x0(dimension), x1(dimension), x2(dimension),
          n1(dimension), n2(dimension);
	...
	// "\n";  std::cerr << "pos1: " <<
	// divCell->wall(k2)->vertex2()->position(1)
	// << "\n";  std::cerr << "wL1: " << wL1 << "\n";
	...
	if (denominator != 0.0) {
	  ...
	  while (divCell->vertex(vk) != v1 && divCell->vertex(vk) != v2) ++vk;
	  std::vector<Vertex *> vertices;
	  vertices.push_back(divCell->vertex(vk++));
	  while (divCell->vertex(vk) != v1 && divCell->vertex(vk) != v2) {
	    if (divCell->vertex(vk) == v1Other || divCell->vertex(vk) == v2Other)
	      oppositeVolumeFlag++;
	    vertices.push_back(divCell->vertex(vk++));
	  }
	  vertices.push_back(divCell->vertex(vk));
	  ...
	  A2 = 0.5 * std::fabs(A2);
	  if (oppositeVolumeFlag) A2 = ACell - A2;
	  if (A2 > Ahalf)
	  ...
	  x0[0] = x1[0] + n1[0] * t1;
        ...
        double root = std::sqrt(
				std::fabs((A0 + Ahalf - A2) /
					  (std::cos(alpha) * std::sin(alpha))));  // not needed?
        // added fabs of argument \andre
        double t1A = std::sqrt((x1[0] - x0[0]) * (x1[0] - x0[0]) +
                               (x1[1] - x0[1]) * (x1[1] - x0[1]));
        double t2A = std::sqrt((x2[0] - x0[0]) * (x2[0] - x0[0]) +
                               (x2[1] - x0[1]) * (x2[1] - x0[1]));
        // double length = 2*(2*t1A+root)*std::cos(alpha);
	
        // std::cerr << "A0: " << A0 << " Ahalf: " << Ahalf << " A2: " << A2 <<
        // "\n";  std::cerr << "root**2: " <<
        // (A0+Ahalf-A2)/(std::cos(alpha)*std::sin(alpha)) << "\n";  std::cerr
        // << "root: " << root << "\n";  std::cerr << "t1A: " << t1A << "\n";
        // std::cerr << "root: " << root << "\n";
	
        std::cerr << divCell->index() << " " << k << " " << k2 << "\t" << wL1
                  << " " << t1A + root << " (" << t1A - root << ")  " << wL2
                  << " " << t2A + root << " (" << t2A - root << ")"
                  << std::endl;
	} else {
	}
	// double tmpLength =
	// divCell->wall(k)->setLengthFromVertexPosition(vertexData);  if(
	// tmpLength > maxLength ) {  wI=k;  maxLength = tmpLength;
      }
    }
    // exit(0);
	 ...
	 DataMatrix &vertexDerivs)
  {
    ...
    if (numVariableIndexLevel() == 2) {
      ...
      std::cerr << "Cell age at division is " << age << std::endl;
  ...
  std::vector<ShortestPath2D::Candidate> ShortestPath2D::
  getCandidates(
		Tissue *T, size_t i, DataMatrix &cellData, DataMatrix &wallData,
		DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
		DataMatrix &vertexDerivs) {
	...
	// std::cerr << " x2 = (" << x2x << ", " << x2y <<
	// ")" << std::endl;
	// std::cerr << " x1p = (" << x1px << ", " << x1py
	// << ")" << std::endl;
	// std::cerr << " x2p = (" << x2px << ", " << x2py
	// << ")" << std::endl;
	// std::cerr << " v = (" << vx << ", " << vy << ")"
	// << std::endl;
	// std::cerr << " u = (" << ux << ", " << uy << ")"
  ...
  double ShortestPath2D::astar(double sigma, double A, double B) {
    double a = 0;
    double b = myMath::pi();
    double e = b - a;
  ...
  ShortestPath::ShortestPath(std::vector<double> &paraValue,
	...
	(indValue.size() == 3 && indValue[2].size() != 2)) {
      std::cerr << "DivisionShortestPath::DivisionShortestPath() "
		...
		<< "variables are used.\n"
		<< "Second level (optional): Cell time index."
		<< "Third level (if centerTriangulated): first:Com index, "
		<< "second: wall length index"
		<< std::endl;
      exit(EXIT_FAILURE);
    }
    
    setId("Division::ShortestPath");
    setNumChange(1);
    setParameter(paraValue);
    setVariableIndex(indValue);
	 ...
	 DataMatrix &vertexDerivs)
  {
	...
	// area +=std::sqrt(temp[0]*temp[0]+temp[1]*temp[1]+temp[2]*temp[2])/2;
      ...
      // rotation matrix for going to the perpendicular plane to thr averaged
      // normal
	...
	cellData[i][variableIndex(2, 0) + 1] =
          rot[1][0] * COMTmp[0] + rot[1][1] * COMTmp[1] + rot[1][2] * COMTmp[2];
	cellData[i][variableIndex(2, 0) + 2] =
          rot[2][0] * COMTmp[0] + rot[2][1] * COMTmp[1] + rot[2][2] * COMTmp[2];
      }
    ...
    if (vertexData[0].size() == 3) {
      ...
      // std::cerr<<std::endl;
      // std::cerr<<" v1w1 "<<vertexData[V1W1][0]<<"  "<<vertexData[V1W1][1]<<"
      // "<<vertexData[V1W1][2]<<std::endl; std::cerr<<" v2w1
      // "<<vertexData[V2W1][0]<<"  "<<vertexData[V2W1][1]<<"
      // "<<vertexData[V2W1][2]<<std::endl; std::cerr<<" v1w2
      // "<<vertexData[V1W2][0]<<"  "<<vertexData[V1W2][1]<<"
      // "<<vertexData[V1W2][2]<<std::endl; std::cerr<<" v2w2
      // "<<vertexData[V2W2][0]<<"  "<<vertexData[V2W2][1]<<"
      // "<<vertexData[V2W2][2]<<std::endl; std::cerr<<" p "<<p[0]<<"  "<<p[1]<<"
      // "<<p[2]<<std::endl; std::cerr<<" q "<<q[0]<<"  "<<q[1]<<"
      // "<<q[2]<<std::endl; std::cerr<<" com "<<COMTmp[0]<<"  "<<COMTmp[1]<<"
      // "<<COMTmp[2]<<std::endl;
      ...
      // for(size_t iii=0; iii< 3; ++iii)
      //   std::cerr<<"rot   "<<rotTemp[iii][0]<<" "<<rotTemp[iii][1]<<"
      //   "<<rotTemp[iii][2]<<std::endl;
	...
	wallVec[k] = vertexData[V2W1][k] - vertexData[V1W1][k];
      ...
      double normW = std::sqrt(wallVec[0] * wallVec[0] + wallVec[1] * wallVec[1] +
			       wallVec[2] * wallVec[2]);
      for (size_t k = 0; k < 3; ++k)
	if (normW != 0)
	  wallVec[k] /= normW;
	else {
	  std::cerr << " in Division::ShortestPath first wall vector is wrong"
		    << std::endl;
	  exit(EXIT_FAILURE);
	}
      
      // projecting the vector between first wall vertex and p on wall vector
      double norm1 = tmpVec[0] * wallVec[0] + tmpVec[1] * wallVec[1] +
	tmpVec[2] * wallVec[2];
	...
	std::cerr << " in Division::ShortestPath p is wrong" << std::endl;
	exit(EXIT_FAILURE);
      }
	...
	wallVec[k] = vertexData[V2W2][k] - vertexData[V1W2][k];
      ...
      normW = std::sqrt(wallVec[0] * wallVec[0] + wallVec[1] * wallVec[1] +
			wallVec[2] * wallVec[2]);
      for (size_t k = 0; k < 3; ++k)
	if (normW != 0)
	  wallVec[k] /= normW;
	else {
	  std::cerr << " in Division::ShortestPath second wall vector is wrong"
		    << std::endl;
	  exit(EXIT_FAILURE);
	}
      
      // projecting the vector between second wall vertex and q on wall vector
      norm1 = tmpVec[0] * wallVec[0] + tmpVec[1] * wallVec[1] +
	tmpVec[2] * wallVec[2];
	...
	T->divideCellCenterTriangulation(
					 &cell, winner.wall1, winner.wall2, variableIndex(2, 0),
					 variableIndex(2, 1), p, q, cellData, wallData, vertexData, cellDerivs,
					 wallDerivs, vertexDerivs, variableIndex(0), parameter(2),
					 parameter(5));
      else {
	std::cerr << "Division::ShortestPath.date(): "
		  << "parameter(5) should be 0 or 1" << std::endl;
	exit(EXIT_FAILURE);
      }
    }
    else // not centerTriangulation
      T->divideCell(&cell, winner.wall1, winner.wall2, p, q, cellData, wallData,
		    vertexData, cellDerivs, wallDerivs, vertexDerivs,
		    variableIndex(0), parameter(2));
    size_t numWallTmp = wallData.size();
  ...
  std::vector<ShortestPath::Candidate> ShortestPath::
  getCandidates(
		Tissue *T, size_t i, DataMatrix &cellData, DataMatrix &wallData,
		DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
		DataMatrix &vertexDerivs) {
...
STAViaShortestPath::STAViaShortestPath(
    std::vector<double> &paraValue,
    std::vector<std::vector<size_t>> &indValue) {
  if (paraValue.size() != 4 && paraValue.size() != 6) {
    std::cerr
        << "DivisionSTAViaShortestPath::DivisionSTAViaShortestPath() "
        << "Four or six parameters are used V_threshold, Lwall_fraction, "
        << "Lwall_threshold, and COM (1 = COM, 0 = Random) " << std::endl
        << "If six parameters are used, two additional parameters are for "
        << "centerTriangulationFlag (=1, or not=0) and double resting length "
           "(1: double, 0:single). "
        << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  if ((paraValue.size() == 4 && indValue.size() != 2) ||
    ...
    std::cerr << "DivisionSTAViaShortestPath::DivisionSTAViaShortestPath() "
              << "First level: Variable indices for volume dependent cell "
                 "variables are used.\n"
              << "Second level: [Next division volume] and [Cell time index] "
                 "(optional)."
              << "Third level (optional, if centerTriangulated): [Com index] "
                 "[wall length index]"
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
int STAViaShortestPath::flag(Tissue *T, size_t i, DataMatrix &cellData,
                             DataMatrix &wallData, DataMatrix &vertexData,
                             DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                             DataMatrix &vertexDerivs) {
  if (T->cell(i).calculateVolume(vertexData) >
      cellData[i][variableIndex(1, 0)]) {
    return 1;
  } else {
    return 0;
  }
...
void STAViaShortestPath::update(Tissue *T, size_t i, DataMatrix &cellData,
                                DataMatrix &wallData, DataMatrix &vertexData,
                                DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                DataMatrix &vertexDerivs) {
  Cell &cell = T->cell(i);
  ...
  // if (vertexData[0].size() != 2)
  //   {
  //     std::cerr << "Division::STAViaShortestPath only supports two
  //     dimensions.\n"; std::exit(EXIT_FAILURE);
  //   }
  ...
  if (vertexData[0].size() == 3) {
    ...
    for (size_t k = 0; k < 3; ++k)
      if (normW != 0)
        ...
        std::cerr << " in DivisionSTAViaShortestPath first wall vector is wrong"
                  << std::endl;
        exit(0);
      }
    ...
    for (size_t k = 0; k < 3; ++k)
      if (normW != 0)
        ...
        std::cerr
            << " in DivisionSTAViaShortestPath second wall vector is wrong"
            << std::endl;
        exit(0);
      }
  ...
  if (numParameter() == 6 && parameter(4) == 1) {  // centerTriangulation
    if (parameter(5) == 0 || parameter(5) == 1)
      ...
      std::cerr << "parameter(5) should be 0 or 1" << std::endl;
      exit(-1);
    }
  } else
    T->divideCell(&cell, winner.wall1, winner.wall2, p, q, cellData, wallData,
...
std::vector<STAViaShortestPath::Candidate> STAViaShortestPath::getCandidates(
    Tissue *T, size_t i, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs) {
  Cell cell = T->cell(i);
...
FlagResetShortestPath::FlagResetShortestPath(
    std::vector<double> &paraValue,
    std::vector<std::vector<size_t>> &indValue) {
  if (paraValue.size() != 4 && paraValue.size() != 6) {
    std::cerr
        << "DivisionFlagResetShortestPath::DivisionFlagResetShortestPath() "
        << "Four or six parameters are used noise amplitude for variable "
           "resetting, Lwall_fraction, "
        << "Lwall_threshold, and COM (1 = COM, 0 = Random) "
        << "If six parameters are used, two additional parameters are for "
        << "centerTriangulation(1) and double resting length (1: double, "
           "0:single). "
        << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  if ((indValue.size() == 2 && indValue[1].size() != 1) ||
    ...
    std::cerr
        << "DivisionFlagResetShortestPath::DivisionFlagResetShortestPath() "
        << "First level: Variable indices for volume dependent cell variables "
           "are used.\n"
        << "Second level (optional): Cell time index."
        << "Third level (if centerTriangulated): first:Com index, second: wall "
           "length index"
        << std::endl;
    exit(EXIT_FAILURE);
  }
...
int FlagResetShortestPath::flag(Tissue *T, size_t i, DataMatrix &cellData,
                                DataMatrix &wallData, DataMatrix &vertexData,
                                DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                DataMatrix &vertexDerivs) {
...
void FlagResetShortestPath::update(Tissue *T, size_t i, DataMatrix &cellData,
                                   DataMatrix &wallData, DataMatrix &vertexData,
                                   DataMatrix &cellDerivs,
                                   DataMatrix &wallDerivs,
                                   DataMatrix &vertexDerivs) {
  ...
  // if (vertexData[0].size() != 2)
  //   {
  //     std::cerr << "Division::FlagResetShortestPath only supports two
  //     dimensions.\n"; std::exit(EXIT_FAILURE);
  //   }
  ...
  if (vertexData[0].size() == 3) {
    ...
    for (size_t k = 0; k < 3; ++k)
      if (normW != 0)
        ...
        std::cerr
            << " in DivisionFlagResetShortestPath first wall vector is wrong"
            << std::endl;
        exit(0);
      }
    ...
    for (size_t k = 0; k < 3; ++k)
      if (normW != 0)
        ...
        std::cerr
            << " in DivisionFlagResetShortestPath second wall vector is wrong"
            << std::endl;
        exit(0);
      }
  ...
  if (numParameter() == 6 && parameter(4) == 1) {  // centerTriangulation
    ...
    // std::cerr<<"divided conc "<<cellData[(T ->
    // numCell())-1][5]<<"\n"<<std::endl;
    // std::cerr<<"X divided conc "<<cellData[(T ->
    // numCell())-2][5]<<"\n"<<std::endl;
    ...
    // inheriting the mother cell index from the lineage indexing before it is
    // updated
...
FlagResetShortestPath::getCandidates(Tissue *T, size_t i, DataMatrix &cellData,
                                     DataMatrix &wallData,
                                     DataMatrix &vertexData,
                                     DataMatrix &cellDerivs,
                                     DataMatrix &wallDerivs,
  ...
  for (size_t i = 0; i < cell.numWall() - 1; ++i) {
    for (size_t j = i + 1; j < cell.numWall(); ++j) {
      ...
      //      std::cerr << " x1p = (" << x1px << ", " << x1py << ")" <<
      //      std::endl; std::cerr << " x2p = (" << x2px << ", " << x2py << ")"
      //      << std::endl;
...
double FlagResetShortestPath::astar(double sigma, double A, double B) {
  ...
  // for (size_t k = 0; k < 10; ++k) {//HJ: ? Why the loop, it returns at first
  // instance
  e = 0.5 * e;
  c = a + e;
  double w = f(c, sigma, A, B);
...
Random::Random(std::vector<double> &paraValue,
               std::vector<std::vector<size_t>> &indValue) {
  if (paraValue.size() != 3) {
    std::cerr << "DivisionRandom::DivisionRandom() "
              << "Three parameters are used V_threshold, Lwall_fraction, "
                 "Lwall_threshold."
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
int Random::flag(Tissue *T, size_t i, DataMatrix &cellData,
                 DataMatrix &wallData, DataMatrix &vertexData,
                 DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                 DataMatrix &vertexDerivs) {
  if (T->cell(i).calculateVolume(vertexData) > parameter(0)) {
...
void Random::update(Tissue *T, size_t i, DataMatrix &cellData,
                    DataMatrix &wallData, DataMatrix &vertexData,
                    DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                    DataMatrix &vertexDerivs) {
  Cell cell = T->cell(i);
  ...
  // Find first vertex.
  {
    ...
    p[0] =
        vertexData[vertex1->index()][0] +
        r * (vertexData[vertex2->index()][0] - vertexData[vertex1->index()][0]);
    p[1] =
        vertexData[vertex1->index()][1] +
        r * (vertexData[vertex2->index()][1] - vertexData[vertex1->index()][1]);
  }
  ...
  T->divideCell(&cell, wall1Index, wall2Index, p, q, cellData, wallData,
                vertexData, cellDerivs, wallDerivs, vertexDerivs,
                variableIndex(0), parameter(2));
...
MainAxis::MainAxis(std::vector<double> &paraValue,
  ...
  //
  if (paraValue.size() != 4) {
    std::cerr << "Division::MainAxis::MainAxis() "
              << "Four parameters are used V_threshold, Lwall_fraction, "
                 "Lwall_threshold, and direction flag (0 = perpendicular to "
                 "main axis, 1 = parallel to main axis)\n";
    exit(EXIT_FAILURE);
  }
...
int MainAxis::flag(Tissue *T, size_t i, DataMatrix &cellData,
                   DataMatrix &wallData, DataMatrix &vertexData,
                   DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                   DataMatrix &vertexDerivs) {
  if (T->cell(i).calculateVolume(vertexData) > parameter(0)) {
...
void MainAxis::update(Tissue *T, size_t cellI, DataMatrix &cellData,
                      DataMatrix &wallData, DataMatrix &vertexData,
                      DataMatrix &cellDeriv, DataMatrix &wallDeriv,
                      DataMatrix &vertexDeriv) {
  ...
  if (dimension != 2) {
    std::cerr << "Division::MainAxis.update() only supports two dimensions."
	      << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  for (size_t i = 0, e = cell.numWall(); i < e; ++i) {
    ...
    double s = ((ax - com[0]) * n[1] - (ay - com[1]) * n[0]) /
               ((ax - bx) * n[1] - (ay - by) * n[0]);
  ...
  if (candidates.size() < 2) {
    std::cerr << "Division::MainAxis.update(): Unable to find enough candidates."
	      << std::endl;
    exit(EXIT_FAILURE);
  }

  if (candidates.size() > 2) {
    std::cerr << "Division::MainAxis.update(): Warning, more than two candidates could be "
	      << "found." << std::endl;
  }
  ...
  T->divideCell(&cell, c1.index, c2.index, c1.p, c2.p, cellData, wallData,
                vertexData, cellDeriv, wallDeriv, vertexDeriv, variableIndex(0),
                parameter(2));
...
VolumeRandomDirectionGiantCells::VolumeRandomDirectionGiantCells(
    std::vector<double> &paraValue,
    std::vector<std::vector<size_t>> &indValue) {
  // Do some checks on the parameters and variable indeces
  //////////////////////////////////////////////////////////////////////
  if (paraValue.size() != 5) {
    std::cerr << "DivisionVolumeRandomDirectionGiantCells::"
              ...
              << "Four parameters used V_threshold, LWall_fraction, "
                 "Lwall_threshold, COM (1 = COM, 0 = Random), and giant cell "
                 "factor to V_threshold.\n";
    std::exit(EXIT_FAILURE);
  }
...
int VolumeRandomDirectionGiantCells::flag(
    Tissue *T, size_t i, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDerivs, DataMatrix &wallDerivs,
    DataMatrix &vertexDerivs) {
  if (cellData[i][variableIndex(1, 0)] &&
      T->cell(i).calculateVolume(vertexData) > parameter(0) * parameter(4)) {
    std::cerr << "Giant Cell " << i << " marked for division with volume "
              << T->cell(i).volume() << std::endl;
    return 1;

  } else if (!cellData[i][variableIndex(1, 0)] &&
             T->cell(i).calculateVolume(vertexData) > parameter(0)) {
    std::cerr << "Cell " << i << " marked for division with volume "
              << T->cell(i).volume() << std::endl;
    return 1;
  }
...
void VolumeRandomDirectionGiantCells::update(
    Tissue *T, size_t cellI, DataMatrix &cellData, DataMatrix &wallData,
    DataMatrix &vertexData, DataMatrix &cellDeriv, DataMatrix &wallDeriv,
    DataMatrix &vertexDeriv) {
  Cell *divCell = &(T->cell(cellI));
...
ShortestPathGiantCells::ShortestPathGiantCells(
    std::vector<double> &paraValue,
    std::vector<std::vector<size_t>> &indValue) {
  if (paraValue.size() != 5) {
    std::cerr
        << "DivisionShortestPathGiantCells::DivisionShortestPathGiantCells() "
        << "Five parameters are used V_threshold, Lwall_fraction, "
           "Lwall_threshold, COM (1 = COM, 0 = Random), and giant cell factor "
           "to V_threshold.\n";
    std::exit(EXIT_FAILURE);
  }
...
int ShortestPathGiantCells::flag(Tissue *T, size_t i, DataMatrix &cellData,
                                 DataMatrix &wallData, DataMatrix &vertexData,
                                 DataMatrix &cellDerivs, DataMatrix &wallDerivs,
                                 DataMatrix &vertexDerivs) {
  if (cellData[i][variableIndex(1, 0)] &&
...
void ShortestPathGiantCells::update(Tissue *T, size_t i, DataMatrix &cellData,
                                    DataMatrix &wallData,
                                    DataMatrix &vertexData,
                                    DataMatrix &cellDerivs,
                                    DataMatrix &wallDerivs,
  ...
  if (vertexData[0].size() != 2) {
    std::cerr
        << "DivisionShortestPathGiantCells only supports two dimensions.\n";
    std::exit(EXIT_FAILURE);
  }
...
ShortestPathGiantCells::getCandidates(Tissue *T, size_t i, DataMatrix &cellData,
                                      DataMatrix &wallData,
                                      DataMatrix &vertexData,
                                      DataMatrix &cellDerivs,
                                      DataMatrix &wallDerivs,
...
FlagResetViaLongestWall::FlagResetViaLongestWall(
    std::vector<double> &paraValue,
    std::vector<std::vector<size_t>> &indValue) {
  //
  // Do some checks on the parameters and variable indeces
...
int FlagResetViaLongestWall::flag(Tissue *T, size_t i, DataMatrix &cellData,
                                  DataMatrix &wallData, DataMatrix &vertexData,
                                  DataMatrix &cellDerivs,
                                  DataMatrix &wallDerivs,
                                  DataMatrix &vertexDerivs) {
...
void FlagResetViaLongestWall::update(Tissue *T, size_t i, DataMatrix &cellData,
                                     DataMatrix &wallData,
                                     DataMatrix &vertexData,
                                     DataMatrix &cellDeriv,
                                     DataMatrix &wallDeriv,
